...
int am_hdcp_work(void *data)
{
	...
	do {
		...
		case HDCP22_SUCCESS:
			...
			msleep_interruptible(200);
			if (irq_clr == 1) {
				DRM_INFO("hdcp22 success\n");
				hdmitx_wr_reg(HDMITX_TOP_INTR_STAT_CLR,
					      0x0000001f);
		...
		case HDCP22_FAIL:
			...
			/*Because the process of "hdcp_tx22"
			 *will deal with HDCP22 Fail, the process here
			 *will get result only.
			 */
			irq_clr = 1;
