...
struct meson_domain {
	...
	struct meson_gpio_irq_resource irq_res;
...
#define IS_EE_DOMAIN(typ) (typ == EE_DOMAIN)
#define IS_AO_DOMAIN(typ) (typ == AO_DOMAIN)

#define CMD_TEST_N_DIR 0x82000046
#define TEST_N_OUTPUT  1

#define PIN(x, b)	(b + x)
#define MESON_MUX_MASK(x) (0xf << x)
#define MESON_MUX_VAL(y, x) ((y & 0xf) << x)
...
extern struct meson_pinctrl_data  meson_txhd_periphs_pinctrl_data;
extern struct meson_pinctrl_data  meson_txhd_aobus_pinctrl_data;

extern int meson_txhd_aobus_init(struct meson_pinctrl *pc);
