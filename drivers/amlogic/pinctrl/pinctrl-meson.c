	...
	for (i = 0; i < pc->data->num_pins; i++) {
		...
		if (pin->pin.number == offset) {
			...
			regmap_update_bits(domain->reg_mux, pin->reg * 4,
				MESON_MUX_MASK(pin->bit),
				MESON_MUX_VAL(0, pin->bit));
			break;
		}
	}
...
static int meson_to_irq(struct gpio_chip *chip, unsigned int gpio)
{
	struct meson_domain *domain = to_meson_domain(chip);

	return irq_find_mapping(domain->irq_res.irqdomain, gpio);
}
...
{
	struct meson_domain *domain = (struct meson_domain *)irqd->chip_data;
...
{
	struct meson_domain *domain = (struct meson_domain *)irqd->chip_data;
	enum meson_domain_type domtype = domain->data->type;
	struct meson_bank *bank;
	struct irq_data *parent_data;
	unsigned long flags;
	...
	for (type_cnt = 0; type_cnt < type_num; type_cnt++) {
		...
		if (domain->irq_res.irq_num == cnt) {
			pr_err("meson_pinctrl: no more gpio irq available in %s GPIO INTC, allocate gpio irq for pin[%ld] failed.\n",
				(IS_EE_DOMAIN(domtype)) ? "EE" : "AO",
				irqd->hwirq);
			return -EINVAL;
		}
		...
		if (IS_EE_DOMAIN(domtype)) {
			...
			regmap_update_bits(domain->reg_irq,
				(cnt < 4) ?
				(GPIO_IRQ_MUX_0_3 * 4) : (GPIO_IRQ_MUX_4_7 * 4),
				0xff << start_bit,
				irq_pin << start_bit);
		} else {
			start_bit = cnt << 2;
		...
		gpio_virq = irq_find_mapping(domain->irq_res.irqdomain,
			domain->irq_res.gpio_irq[cnt].hwirq);
...
struct meson_pinctrl_private meson_txhd_periphs = {
	.pinctrl_data = &meson_txhd_periphs_pinctrl_data,
	.init = NULL,
};

struct meson_pinctrl_private meson_txhd_aobus = {
	.pinctrl_data = &meson_txhd_aobus_pinctrl_data,
	.init = meson_txhd_aobus_init,
};

static const struct of_device_id meson_pinctrl_dt_match[] = {
	{
		.compatible = "amlogic,meson-txhd-periphs-pinctrl",
		.data = &meson_txhd_periphs,
	},
	{
		.compatible = "amlogic,meson-txhd-aobus-pinctrl",
		.data = &meson_txhd_aobus,
	},
	{ },
};
...
static int meson_gpiolib_register(struct meson_pinctrl *pc)
{
	...
	domain->chip.set_pullup_down = meson_gpio_pull_set;
	...
	return 0;
fail:
	if (gpiochip_remove(&pc->domain->chip))
		dev_err(pc->dev, "remove gpiochip failed\n");
					...
					struct device_node *node)
{
	...
	domain->irq_res.gpio_irq = devm_kzalloc(pc->dev,
		sizeof(struct meson_irq_desc)*(domain->irq_res.irq_num),
		GFP_KERNEL);
...
static void meson_gpio_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	struct irq_chip *chip = irq_desc_get_chip(desc);
	struct meson_domain *domain = irq_desc_get_handler_data(desc);
	unsigned int parent_virq  = irq;
	unsigned char cnt;
	...
	for (cnt = 0; cnt < domain->irq_res.irq_num; cnt++) {
		if (parent_virq == domain->irq_res.gpio_irq[cnt].parent_virq &&
			...
			generic_handle_irq(
				irq_find_mapping(domain->irq_res.irqdomain,
				domain->irq_res.gpio_irq[cnt].hwirq));
	}
...
static int meson_irq_setup(struct meson_pinctrl *pc)
{
	...
	domain->irq_res.irqdomain = irq_domain_add_linear(domain->of_node,
		domain->data->num_pins, &irq_domain_simple_ops, NULL);
	if (!domain->irq_res.irqdomain) {
		dev_err(pc->dev, "fail to create irq domain.\n");
		return -ENOMEM;
	}

	for (cnt = 0; cnt < domain->data->num_pins; cnt++) {
		irqno = irq_create_mapping(domain->irq_res.irqdomain, cnt);
		irq_set_chip_and_handler(irqno,
			&meson_gpio_irq_chip, handle_simple_irq);
		irq_set_chip_data(irqno, domain);
	}
...
static int __init meson_pmx_init(void)
{
	return platform_driver_register(&meson_pinctrl_driver);
}

static void __exit meson_pmx_exit(void)
{
	platform_driver_unregister(&meson_pinctrl_driver);
}

arch_initcall(meson_pmx_init);
module_exit(meson_pmx_exit);
MODULE_DESCRIPTION("meson pin control driver");
