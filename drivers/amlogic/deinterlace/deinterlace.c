		...
		queue_for_each_entry(buf_p, ptmp, QUEUE_POST_FREE, list) {
			if (buf_p->pages == NULL) {
				buf_p->pages =
					...
					&(de_devp->pdev->dev),
					de_devp->post_buffer_size>>PAGE_SHIFT,
					0);
				if (IS_ERR_OR_NULL(buf_p->pages)) {
					buf_p->pages = NULL;
					pr_err("xxxxxxxxx DI CMA allocate %d fail.\n",
	...
	int frame_type, int wWidth)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_GXLX))
		adaptive_cue_adjust(field_pd_info->frame_diff_num,
...
static void di_get_vpu_clkb(struct device *dev, struct di_dev_s *pdev)
{
	...
	vpu_clk = clk_get(dev, "cts_vpu_clk");
	if (IS_ERR(vpu_clk))
		pr_err("%s: get clk vpu error.\n", __func__);
	vpu_clkb_tmp = clk_get(dev, "cts_vpu_clkb_tmp");
	if (IS_ERR(vpu_clkb_tmp))
		pr_err("%s: get vpu clkb tmp error.\n", __func__);
	pdev->vpu_clkb = clk_get(dev, "cts_vpu_clkb");
...
MODULE_PARM_DESC(post_wr_support, "\n post write enable\n");
module_param(post_wr_support, uint, 0664);
...
module_param(nr10bit_support, uint, 0664);
MODULE_PARM_DESC(nr10bit_support, "\n nr10bit support flag\n");
...
module_param_named(show_nrwr, show_nrwr, bool, 0644);
#endif
