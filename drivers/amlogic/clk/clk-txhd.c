/*
 * drivers/amlogic/clk/clk-txhd.c
 *
...
/* spicc */
#define	HHI_SPICC_CLK_CNTL		OFFSET(0xf7)
/* tcon */
#define	HHI_TCON_CLK_CNTL		OFFSET(0xf0)
/* ts */
#define	HHI_TS_CLK_CNTL			OFFSET(0xef)

#define TXHD_RSTC_N_REGS	6
#define TXHD_AO_OFF		((TXHD_RSTC_N_REGS - 1) * BITS_PER_LONG + 4)
...
PNAME(cts_spicc_p) = {"xtal", "clk81", "fclk_div4", "fclk_div3",
		"fclk_div2", "fclk_div5", "fclk_div7", "gp0_pll"};
PNAME(cts_tcon_p) = {"xtal", "fclk_div5", "fclk_div4", "fclk_div3",
		"mpll_clk_out2", "mpll_clk_out3", "vid_pll_clk", "gp0_pll"};
PNAME(cts_ts_p) = {"xtal", "clk81", "fclk_div7", "fclk_div5"};
...
static struct amlogic_fixed_rate_clock txhd_fixed_rate_ext_clks[] __initdata = {
...
static struct amlogic_clk_branch clk_branches[] __initdata = {
	...
	/* cts_spicc_clk*/
	COMPOSITE(CLK_SPICC, "cts_spicc_clk", cts_spicc_p,
		   CLK_SET_RATE_NO_REPARENT,
		   HHI_SPICC_CLK_CNTL, 7, 3, 0,
		   HHI_SPICC_CLK_CNTL, 0, 6,
		   CLK_DIVIDER_ROUND_CLOSEST,
		   HHI_SPICC_CLK_CNTL, 6, 0),
	/* cts_tcon_clk*/
	COMPOSITE(CLK_TCON, "cts_tcon_clk", cts_tcon_p,
		   CLK_SET_RATE_NO_REPARENT,
		   HHI_TCON_CLK_CNTL, 7, 3, 0,
		   HHI_TCON_CLK_CNTL, 0, 6,
		   CLK_DIVIDER_ROUND_CLOSEST,
...
/* register txhd clocks */
static void __init txhd_clk_init(struct device_node *np)
{
	...
	amlogic_clk_of_register_fixed_ext(txhd_fixed_rate_ext_clks,
		  ARRAY_SIZE(txhd_fixed_rate_ext_clks), ext_clk_match);
	...
	meson_register_rstc(np, TXHD_RSTC_N_REGS, reg_base_aobus,
		reg_base_hiubus + HHI_GCLK_MPEG0, TXHD_AO_OFF, 0);
...
CLK_OF_DECLARE(txhd, "amlogic, txhd-clock", txhd_clk_init);
