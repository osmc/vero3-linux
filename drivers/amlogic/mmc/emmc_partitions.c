...
static unsigned char *dtb_write_buf;
static DEFINE_MUTEX(dtb_write_mutex);
		...
		size_t count, loff_t *ppos)
{
	...
	if (mutex_lock_interruptible(&dtb_write_mutex))
		return -EINTR;
	...
	memset(dtb_write_buf, 0, CONFIG_DTB_SIZE);
	ret = copy_from_user(dtb_write_buf + *ppos, buf, write_size);
	if (ret) {
		write_size = -EFAULT;
		goto exit;
	}

	ret = amlmmc_dtb_write(card_dtb, dtb_write_buf, CONFIG_DTB_SIZE);
	if (ret) {
...
int amlmmc_dtb_init(struct mmc_card *card)
{
	...
	/*
	 * Pre-allocate the write buffer here at init time, before memory
	 * becomes fragmented by CMA users (e.g. video). A 256KB order-6
	 * kmalloc fails reliably on high-uptime systems where the Amlogic
	 * CMA compaction hooks block the buddy allocator from coalescing
	 * pages of that order. The buffer must be physically contiguous
	 * because mmc_transfer() passes it directly to sg_init_one() for DMA.
	 */
	dtb_write_buf = kmalloc(CONFIG_DTB_SIZE, GFP_KERNEL);
	if (!dtb_write_buf)
		return -ENOMEM;
