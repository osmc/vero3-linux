...
static bool skip_sd_probe;
core_param(skip_sd_probe, skip_sd_probe, bool, 0444);
	...
	case HOST_DAT_CRC_ERR:
		if (host->is_tunning == 0) {
			...
			case HOST_DAT_CRC_ERR:
				...
				break;
			case HOST_RSP_TIMEOUT_ERR:
				host->err_rsp_timeout++;
				break;
			case HOST_DAT_TIMEOUT_ERR:
				host->err_dat_timeout++;
				break;
			default:
				break;
			}
...
/*
 * OSMC: report the cumulative controller error / recovery counters.
 * These surface transient CRC/timeout events the driver recovers from via
 * its retry/re-tune machinery -- events that never reach the block layer and
 * so never appear in dmesg.  A non-zero "fatal" line means a command
 * exhausted every retry: that is the only case that should also have logged a
 * block-layer error.  Writing anything to the node clears the counters.
 */
static ssize_t emmc_errors_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
		"rsp_crc %u\ndat_crc %u\nrsp_timeout %u\ndat_timeout %u\nfatal %u\n",
		host->err_rsp_crc, host->err_dat_crc,
		host->err_rsp_timeout, host->err_dat_timeout,
		host->err_fatal);
}

static ssize_t emmc_errors_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	host->err_rsp_crc = 0;
	host->err_dat_crc = 0;
	host->err_rsp_timeout = 0;
	host->err_dat_timeout = 0;
	host->err_fatal = 0;

	return len;
}

static DEVICE_ATTR(emmc_errors, 0644, emmc_errors_show, emmc_errors_store);
...
static int meson_mmc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(host->base)) {
       ...
       if (skip_sd_probe && host->data->port_b_base) {
               struct resource *res_check;

               res_check = platform_get_resource(pdev, IORESOURCE_MEM, 0);
               if (res_check
                       && (res_check->start == host->data->port_b_base)) {
                       dev_info(&pdev->dev,
                               "Skipping SD probe for debug UART"
                               " (skip_sd_probe=1)\n");
                       kfree(host);
                       return -ENODEV;
               }
	...
	if (host->ctrl_ver >= 3) {
		...
		if (ret)
			dev_warn(mmc_dev(host->mmc),
				 "Unable to creat sysfs attributes\n");
		ret = device_create_file(&pdev->dev, &dev_attr_emmc_errors);
		if (ret)
			dev_warn(mmc_dev(host->mmc),
				 "Unable to creat sysfs attributes\n");
	}
