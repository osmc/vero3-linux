...
void dmc_set_urgent(unsigned int port, unsigned int type)
{
	...
	if (aml_db->cpu_type < MESON_CPU_MAJOR_ID_G12A) {
		...
		unsigned int port_reg[24] = {
			...
			DMC_AXI10_G12_CHAN_CTRL, DMC_AXI11_G12_CHAN_CTRL,
	...
	/*
	 * SC2 relocated the DMC_MON block from word 0x20 to word 0x50, and
	 * mesonsc2.dtsi compensates by giving this driver a base of DMC+0xc0
	 * so the MON offsets in ddr_band_op_g12.c land correctly. The AM/AXI
	 * channel control registers did not move, so undo that skew here.
	 */
	if (aml_db->cpu_type == MESON_CPU_MAJOR_ID_SC2)
		addr -= 0xc0;
