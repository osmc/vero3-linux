/*
 * drivers/amlogic/display/vout/lcd/lcd_tcon.c
 *
...
void lcd_tcon_regs_table_print(struct mlvds_config_s *mlvds_conf)
{
	...
	buf = kmalloc((PR_BUF_MAX * sizeof(char)), GFP_KERNEL);
	if (buf == NULL) {
...
int lcd_tcon_od_set(int flag)
{
	...
	temp = lcd_tcon_read(0x05c + TCON_SYS_REG_START);
	if (flag)
		temp |= (1 << 6);
	else
		temp &= ~(1 << 6);
	temp &= 0xff;
	lcd_tcon_write((0x05c + TCON_SYS_REG_START), temp);
	LCDPR("%s: %d\n", __func__, flag);
...
int lcd_tcon_probe(struct device *dev)
{
	...
	if ((ret != 0) && ((void *)tcon_fb_rmem.base == NULL))
		LCDERR("failed to init tcon reserved memory\n");
	else
		mlvds_conf->fb_addr = virt_to_phys((void *)tcon_fb_rmem.base);
	LCDPR("tcon fb_addr = 0x%08x\n", mlvds_conf->fb_addr);

	switch (lcd_drv->lcd_config->lcd_basic.v_active) {
	case 768:
	...
	para = kzalloc((sizeof(unsigned char) * LCD_UKEY_TCON_SIZE),
		GFP_KERNEL);
	if (!para) {
