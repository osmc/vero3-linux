...
{
	uint i, j, n, hist[3][64], pix, total_pix;
	...
	while (i < 101)
	{
		...
		pr_info("\t%u\t%u\n", i, 16 * j + (((n - pix) * 32/hist[0][j] + 1) >> 1));
		n = (2 * total_pix * ++i / 100 + 1) >> 1;
	}
	pr_info("\n\t dump_dnlp_hist done\n");
}
		...
		const char *buf, size_t count)
{
	...
	if (val[0] > 4 || val[0] < 0) return;
	WRITE_VPP_REG_BITS(VI_HIST_CTRL, val[0], 11, 3);
