	...
	struct matrix_s *gmt_mtx)
{
	...
	/* Deal with DV */
	if (is_dolby_vision_enable() && module_sel != OSD1_HDR ) {
		if (get_dv_profile() == 5)
			hdr_process_select |= IPT_MAP;
		if (get_dv_profile() == 8 && hdr_source_type == HLG_SOURCE) {
			if (sink_support_dolby_vision(vinfo) & 1)
				hdr_process_select = HLG_IPT;
			else if (sink_hdr_support(vinfo) & HLG_SUPPORT)
				hdr_process_select = HDR_BYPASS;
			else
				hdr_process_select = HLG_HDR;
		}
	}
	...
	pr_csc(8, "am_vecm: hdr module=%s, process=%s (0x%08x)\n",
	       module_str[module_sel],
	       process_str[index], hdr_process_select);
}
	...
	if (get_hdr_module_status(vd_path) != HDR_MODULE_ON &&
	   cur_hdr_policy != 2) {
		...
		if (source_format[vd_path] == HDRTYPE_MVC) {
			...
			cuva_hdr_process_mode[vd_path] = PROC_BYPASS;
			target_format[vd_path] = BT709;
			set_hdr_module_status(vd_path, HDR_MODULE_ON);
	...
	enum hdr_type_e *source_type)
{
	...
	if (get_hdr_module_status(vd_path) == HDR_MODULE_OFF) {
		...
		if (((vd_path == VD1_PATH) &&
		    ...
		    ((vd_path == VD2_PATH) &&
		     !is_video_layer_on(VD1_PATH)))
			hdr_proc(vf, OSD1_HDR, HDR_BYPASS | HDR_SDR, vinfo, NULL);
	...
	case HDRTYPE_DOVI:
		if (vd_path == VD1_PATH && is_dolby_vision_on() && 
			...
			if (get_dv_profile() != 7) {
				...
				hdr_proc(vf, OSD1_HDR, HDR_IPT | RGB_OSD, vinfo, NULL);
			} else {
				hdr_proc(vf, VD1_HDR, HDR_BYPASS, vinfo, NULL);
	...
	case HDRTYPE_CUVA_HDR:
		if (vd_path == VD2_PATH && is_dolby_vision_on()) {
			...
			hdr_proc(vf, OSD1_HDR, HDR_HLG, vinfo, NULL);
		}
