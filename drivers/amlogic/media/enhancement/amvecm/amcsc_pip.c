...
void video_post_process(
	...
	enum hdr_type_e *source_type)
{
	cur_source_format[vd_path] = source_type[vd_path];
	...
	case HDRTYPE_DOVI:
		if (vd_path == VD2_PATH && is_dolby_vision_on()) {
			...
			if (vd_path == VD1_PATH)
				hdr_proc(vf, VD1_HDR, (HDR_BYPASS | IPT_MAP), vinfo, NULL);
			else
				hdr_proc(vf, VD2_HDR, (HDR_BYPASS | IPT_MAP), vinfo, NULL);
			if ((get_hdr_policy() == 2) &&
			    (target_format[vd_path] == BT_BYPASS))
				hdr_proc(vf, OSD1_HDR, HDR_BYPASS, vinfo, NULL);
			else
			...
			eo_clip_proc(master_info, eo_sel);
			if (vd_path == VD1_PATH) {
				hdr_proc(vf, VD1_HDR, (HDR_SDR | IPT_MAP), vinfo, &m);
			} else {
				hdr_proc(vf, VD2_HDR, (HDR_SDR | IPT_MAP), vinfo, &m);
			}
			hdr_proc(vf, OSD1_HDR, HDR_BYPASS, vinfo, NULL);
			target_format[vd_path] = BT709;
		} else if (hdr_process_mode[vd_path] == PROC_HDR_TO_HLG) {
			if (vd_path == VD1_PATH)
				hdr_proc(vf, VD1_HDR, (HDR_HLG || IPT_MAP), vinfo, NULL);
			else
				hdr_proc(vf, VD2_HDR, (HDR_HLG | IPT_MAP), vinfo, NULL);
			hdr_proc(vf, OSD1_HDR, SDR_HLG, vinfo, NULL);
			target_format[vd_path] = BT2020_HLG;
		} else if (hdr_process_mode[vd_path] == PROC_HDR_TO_CUVA) {
			if (vd_path == VD1_PATH)
