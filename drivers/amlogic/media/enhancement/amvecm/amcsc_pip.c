	...
	if (get_hdr_module_status(vd_path) != HDR_MODULE_ON &&
	   cur_hdr_policy != 2) {
		...
		if (source_format[vd_path] == HDRTYPE_MVC) {
			...
			enum output_format_e force_output = get_force_output();
			switch (get_hdr_mode()) {
			case 1:
				force_output = BT709;
				break;
			...
			case HDRTYPE_HLG:
				...
				& HLG_SUPPORT) && (force_output == UNKNOWN_FMT 
				|| force_output == BT2020_HLG)) {
					/* hlg->hlg */
					hlg_process_mode[vd_path] =
						PROC_BYPASS;
					target_format[vd_path] =
				...
				&& (force_output == UNKNOWN_FMT
				|| force_output == BT2020_PQ)) {
					/* hlg->hdr */
					hlg_process_mode[vd_path] =
						PROC_HLG_TO_HDR;
					target_format[vd_path] =
			...
			case HDRTYPE_HDR10:
				/* source HDR10 */
				if ((sink_hdr_support(vinfo)
				& HDR_SUPPORT) && (force_output == UNKNOWN_FMT
				|| force_output == BT2020_PQ)) {
					/* hdr bypass */
			...
			case HDRTYPE_HDR10PLUS:
				...
				&& !is_video_layer_on(oth_path))
				&& (force_output == UNKNOWN_FMT
				|| force_output == BT2020_PQ_DYNAMIC)) {
					/* hdr+ bypass */
					hdr10_plus_process_mode[vd_path] =
						PROC_BYPASS;
					target_format[vd_path] =
