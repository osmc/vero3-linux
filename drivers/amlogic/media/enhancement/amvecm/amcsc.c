...
/* Note: GXL remains 0: limit-limit 1: full-limit 2: limit-full 3: full-full */
static uint range_control;
module_param(range_control, uint, 0664);
MODULE_PARM_DESC(range_control, "\n range_control 0:follow source, limit out 1:limit-limit 2:source-full 3:limit-full\n");
	...
	struct hdr10plus_para *p)
{
	...
	if (vf->src_fmt.fmt == VFRAME_SIGNAL_FMT_HDR10PLUS)
		csc_type = VPP_MATRIX_BT2020YUV_BT2020RGB_DYNAMIC;
	...
	enum vd_path_e vd_path)
{
	...
	if (vf) {
		...
		if (vf->src_fmt.fmt == VFRAME_SIGNAL_FMT_HDR10PLUS
			&& (cur_signal_type[vd_path] & 0x3000) != 0x3000) {
			csc_type = VPP_MATRIX_BT2020YUV_BT2020RGB_DYNAMIC;
			cur_signal_type[vd_path] &= 0xffff00ff;
			cur_signal_type[vd_path] |= 0x00003000;
			get_cur_vd_signal_type(vd_path);
	...
	if (!is_video_layer_on(VD2_PATH) || vd_path == VD2_PATH)
		hdr2_needs_update = 0;
	...
	enum vd_path_e vd_path)
{
	...
	if (vd_path == VD1_PATH || vd_path == VD2_PATH) {
		if (is_dolby_vision_on()) {
			if (!dovi_on)
				hdr_func(vd_path + 1, HDR_BYPASS,
					 get_current_vinfo(),
					 NULL);
		...
		if (null_vf_cnt[vd_path] == toggle_frame) {
			...
			if (send_fake_frame) {
				...
				pr_csc(4,
				       ...
				       vd_path == VD1_PATH ? "VD1" : "VD2");
				if (!is_dolby_vision_enable())
					set_hdr_module_status(VD2_PATH, HDR_MODULE_BYPASS);
				if ((vd_path == VD2_PATH) &&
				    (is_video_layer_on(VD1_PATH) ||
