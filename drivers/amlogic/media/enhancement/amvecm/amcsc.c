...
uint get_range_control(void){
	if (range_control & 2)
		set_vpp_matrix(VPP_MATRIX_OSD, RGB709_to_YUV709_coeff, CSC_ON);
	else
		set_vpp_matrix(VPP_MATRIX_OSD, RGB709_to_YUV709l_coeff, CSC_ON);
	...
	if (get_cpu_type() > MESON_CPU_MAJOR_ID_GXTVBB) {
		...
		if ((csc_type == VPP_MATRIX_BT2020YUV_BT2020RGB) &&
			...
			if (range_control & 2){
				...
				pr_csc(1, "Full range RGB-YUV\n");
			}
			else {
				set_vpp_matrix(VPP_MATRIX_OSD,
					RGB709_to_YUV709l_coeff,
					CSC_ON);	/* use limit range */
				pr_csc(1, "Limited range RGB-YUV\n");
			}
		}
