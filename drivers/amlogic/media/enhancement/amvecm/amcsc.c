...
static void print_primaries_info(struct vframe_master_display_colour_s *p)
{
	...
	pr_csc(1, "\tmax, min luminance = %d, %d / 10000\n",
	...
	struct vinfo_s *vinfo, enum vd_path_e vd_path)
{
	...
	&& ((p_new->present_flag & 0x80000000) == 0)) {
		...
		cus.content_light_level.max_content =
			customer_master_display_param[10];
		cus.content_light_level.max_pic_average =
			customer_master_display_param[11];
		cus.present_flag = 1;
		p_new = &cus;
	}
		...
		if (signal_transfer_characteristic == 16) {
			...
			if (signal_color_primaries == 9) {
				...
				if (tx_hdr10_plus_support)
					...
					pr_csc(1, "No HDR10+ support - using HDR10\n");
					csc_type = VPP_MATRIX_BT2020YUV_BT2020RGB;
				}
			} else {
				pr_csc(1,
	...
	if ((get_cpu_type() > MESON_CPU_MAJOR_ID_GXTVBB)
		...
		if (video_lut_swtich < 8)
			...
			if (vpp_pq_lut_curve_calc(customer_panel_lumin,
				...
				for (i=1; i<4; i++)
					scaler_coeff[4 * i - 1] = 1024 + contrasts[1] * 8;
				set_vpp_matrix(VPP_MATRIX_VD1, scaler_coeff, CSC_ON);
				pr_info("LUT curve calc target %d master %d, contrast1 %d, contrast2 %d\n",
					customer_panel_lumin, max_lumin, amvecm_get_contrast1(), amvecm_get_contrast2());
			} else {
	...
	enum vd_path_e vd_path)
{
	...
	if (get_cpu_type() > MESON_CPU_MAJOR_ID_GXTVBB) {
		...
		if ((csc_type == VPP_MATRIX_BT2020YUV_BT2020RGB) &&
			...
			for (i = 0; i < 9; i++)
				osd_mtx[i] = osd_mtx[i] * max_lumin / 10000;
			...
			set_vpp_lut(VPP_LUT_OSD_OETF,
				osd_oetf_41_2084_mapping_10000,
				osd_oetf_41_2084_mapping_10000,
				osd_oetf_41_2084_mapping_10000,
				CSC_ON);
		...
		if (csc_type == VPP_MATRIX_BT2020YUV_BT2020RGB)
			...
			/* used only for RGB output to LCD */
			if (vinfo->viu_color_fmt == COLOR_FMT_RGB444) {
			...
			if (get_hdr_type() & HLG_FLAG)
				...
				if (hdr_mode == 3) /* hdr-2020sdr */
					send_info.features =
						...
						| (1 << 29) /*video available*/
						| (5 << 26) /* unspecified */
						| (((range_control & 2) ? 1 : 0) << 25) /* full/limit */
						| (1 << 24) /*color available*/
						| (9 << 16) /* bt2020 */
						| (14 << 8)	/* bt2020-10 */
						| (9 << 0); /* bt2020nc */
