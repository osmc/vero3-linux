...
enum vpp_matrix_csc_e get_csc_type(int print)
{
	...
	} else if (signal_color_primaries == 2 && signal_ipt) {
		/* candidate DV, subject to NALU read */
	...
	enum hdr_type_e *source_type)
{
	...
	if ((get_cpu_type() > MESON_CPU_MAJOR_ID_GXTVBB)
		...
		if (vpp_pq_lut_curve_calc(customer_panel_lumin,
...
static int ipt_metadata_update(
	struct vframe_s *vf)
{
	static char last_rpu[512];
	static uint16_t last_rpu_size = 0;
	static uint16_t dm_size = 0;
	parser_dynmic_metadata(vf);
	struct vframe_master_display_colour_s master_info;

	if (vf->src_fmt.md_buf && vf->src_fmt.md_size){
		if (vf->src_fmt.md_size == last_rpu_size) {
			int i, j, len;
			int check = memcmp((void*) vf->src_fmt.md_buf, (void*) last_rpu,
			 vf->src_fmt.md_size - dm_size);
			if (check != 0){
				pr_csc(0x10, "IPT (%i) rpu changed - parsing\n", vf->index_disp);
			} else {
				check = memcmp((void*) (vf->src_fmt.md_buf + vf->src_fmt.md_size - dm_size), (void*) (last_rpu + last_rpu_size - dm_size),
			 		dm_size);
				if (check != 0)
					pr_csc(0x10, "IPT (%i) only dm changed - ignoring\n", vf->index_disp);
				return 0;
			}
			for (i = 0; i < last_rpu_size + 8; i += 8) {
				len = last_rpu_size - i;
				if (len < 8) {
					pr_csc(0x10, "\t i = %02d", i);
					for (j = 0; j < len; j++)
		...
		if (parse_rpu(vf->src_fmt.md_buf, (uint16_t) vf->src_fmt.md_size, (uint16_t*) &dm_size, (uint32_t*) &master_info)) {
			memcpy((void*)last_rpu, (void*) vf->src_fmt.md_buf, vf->src_fmt.md_size);
			memcpy(&(vf->prop.master_display_colour), &master_info, sizeof(struct vframe_master_display_colour_s));
			last_rpu_size = vf->src_fmt.md_size;
			pr_csc(16, "master_info %u %u %u %u\n", master_info.luminance[0], master_info.luminance[1], master_info.white_point[0], master_info.white_point[1]);
			pr_csc(16, "Master info from RPU %i %i %i\n", vf->prop.master_display_colour.present_flag,
				vf->prop.master_display_colour.luminance[0], vf->prop.master_display_colour.luminance[1]);
			return 1;
		}
	}
	pr_csc(0x10, "RPU not found\n");
	return -1;
}
	...
	enum vd_path_e vd_path)
{
	...
	if (vf && ((flags & (CSC_FLAG_TOGGLE_FRAME | CSC_FLAG_FORCE_SIGNAL)))) {
		...
		if ((cur_vd_signal_type & 0xff) < 3) {
			rpu_status = ipt_metadata_update(vf);
		}
		if (rpu_status >= 0){
			csc_type = VPP_MATRIX_BT2020YUV_BT2020RGB;
			cur_vd_signal_type |= 1 << 30;
			source_format[vd_path] = DOVI_SOURCE;
			/* update how we are going to process this */
			hdr_policy_process(vinfo, source_format, vd_path);
			if (rpu_status == 1) {
				hdr10_primaries_changed(&(vf->prop.master_display_colour), &cur_master_display_colour[vd_path]);
				p->luminance[0] *= 10000;
