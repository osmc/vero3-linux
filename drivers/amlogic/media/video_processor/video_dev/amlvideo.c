...
static int video_receiver_event_fun(int type, void *data, void *private_data)
{
	...
	if (type == VFRAME_EVENT_PROVIDER_UNREG) {
		...
		// directly tell DI to release vframes left in q_omx
		...
		// directly tell DI to release vframes left in q_ready
		while ((vf = vfq_peek(&dev->q_ready))) {
			vf = vfq_pop(&dev->q_ready);
			if ((dev->vf->type & VIDTYPE_DI_PW) || (dev->vf->type & VIDTYPE_INTERLACE)) {
				dim_post_keep_cmd_release2(vf);
			}
...
static int vidioc_qbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	while ((vf = vfq_peek(&dev->q_omx))) {
		...
		if (p->index > index) {
			amlvideo_vf_put(vfq_pop(&dev->q_omx), dev);
...
static int vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	if (dev->vf->type & VIDTYPE_V4L_EOS) {
		// EOS?? just return the vframe to the caller
		amlvideo_vf_put(dev->vf, dev);
		mutex_unlock(&dev->vf_mutex);
		return -EAGAIN;
	}
