...
#define enable_video_layer()  \
	do { \
		if (vd_layer[0].global_debug & DEBUG_FLAG_BLACKOUT) {  \
			pr_info("VIDEO: EnableVideoLayer()\n"); \
		} \
		VD1_MEM_POWER_ON(); \
		VIDEO_LAYER_ON(); \
	} while (0)
...
#define enable_video_layer2()  \
	do { \
		if (vd_layer[1].global_debug & DEBUG_FLAG_BLACKOUT) {  \
			pr_info("VIDEO: EnableVideoLayer2()\n"); \
		} \
		VD2_MEM_POWER_ON(); \
		VIDEO_LAYER2_ON(); \
	} while (0)
	...
	struct vframe_s *vf)
{
	...
	type = vf->type;
	if ((type & VIDTYPE_MVC) || is_enable_3d_to_2d() || (process_3d_type & MODE_3D_FP))
		is_mvc = true;
	...
	if (is_mvc) {
		if (framepacking_support || is_need_mvc_to_sbs_output() || is_need_mvc_to_tab_output())
			pat = 0;
		else
			pat = 0x88;
		VSYNC_WR_MPEG_REG(
	...
	/* picture 0/1 control */
	if ((framepacking_support || is_need_mvc_to_sbs_output() || is_need_mvc_to_tab_output()) && mvc_flag) {
		/* frame packed output */
...
static s32 vd1_mif_setting(struct mif_pos_s *setting)
{
	...
	ls = setting->l_hs_luma;
	le = setting->l_he_luma;
	VSYNC_WR_MPEG_REG(
	...
	struct vframe_s *dvel_vf)
{
	...
	setting->postblend_h_end = setting->frame_par->VPP_line_in_length_ - 1;
	setting->postblend_v_start = 0;
	setting->postblend_v_end = setting->frame_par->VPP_pic_in_height_ - 1;

	setting->frame_par->VPP_pic_in_height_ = (dvel_vf->type & VIDTYPE_COMPRESS) ? dvel_vf->compHeight : dvel_vf->height;
	setting->frame_par->VPP_line_in_length_ = (dvel_vf->type & VIDTYPE_COMPRESS) ? dvel_vf->compWidth : dvel_vf->width;

	setting->frame_par->vpp_filter.vpp_hf_start_phase_step >>= 1;
	setting->frame_par->vpp_filter.vpp_vsc_start_phase_step >>= 1;
	...
	u32 *ls, u32 *le, u32 *rs, u32 *re)
{
	...
	if (vpp_3d_mode == VPP_3D_MODE_TB) {
		if (vf->trans_fmt == TVIN_TFMT_3D_FP) {
			...
			*le = *le - 1;
			*re = *re - 1;
		} else if (process_3d_type & MODE_3D_FULL_LRTB) {
			*ls = 0;
			*le = (frame_height >> 1) - 1;
			*rs = frame_height >> 1;
			*re = frame_height - 1;
	...
	struct blend_setting_s *setting)
{
	...
	if ((process_3d_type & MODE_3D_MVC) && is_need_mvc_to_tab_output()) {
		...
		layer->sc_setting.sc_v_enable = true; // turn on VSC for MVC => H-TAB
		...
		setting->postblend_v_end =
			layer->end_y_lines;
	} else if ((dispbuf && (dispbuf->type & VIDTYPE_MVC)) || (process_3d_type & MODE_3D_FP)) {
		u32 blank;
	...
	struct video_layer_s *layer)
{
	...
	if ((process_3d_type & MODE_3D_OUT_TB) || (process_3d_type & MODE_3D_OUT_LR) ||
	    (process_3d_type & MODE_3D_FULL_LRTB)) {
		if (is_need_mvc_to_sbs_output() || is_need_mvc_to_tab_output()) {
			...
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_luma_psel, 0x4000000);
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_chroma_psel, 0x4000000);
			VSYNC_WR_MPEG_REG(vd2_mif_reg->vd_if0_luma_psel, 0);
			VSYNC_WR_MPEG_REG(vd2_mif_reg->vd_if0_chroma_psel, 0);
		} else {
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_luma_psel, 0);
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_chroma_psel, 0);
			VSYNC_WR_MPEG_REG(vd2_mif_reg->vd_if0_luma_psel, 0x4000000);
			VSYNC_WR_MPEG_REG(vd2_mif_reg->vd_if0_chroma_psel, 0x4000000);
		}
	} else if ((process_3d_type & MODE_3D_ENABLE) && cur_frame_par->vpp_2pic_mode == VPP_SELECT_PIC0) {
		VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_luma_psel, 0);
		VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_chroma_psel, 0);
	} else if ((process_3d_type & MODE_3D_ENABLE) && cur_frame_par->vpp_2pic_mode == VPP_SELECT_PIC1) {
		VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_luma_psel, 0x4000000);
		VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_chroma_psel, 0x4000000);
	}
	...
	struct mif_pos_s *setting)
{
	...
	/* framepacking mode, need use right eye position */
	/* to set vd2 Y0 not vd1 Y1*/
	if ((setting->id == 0) && (dispbuf->type & VIDTYPE_MVC) && !is_enable_3d_to_2d() && !framepacking_support) {
		setting->l_vs_chrm = setting->l_vs_luma;
		setting->l_ve_chrm = setting->l_ve_luma;
		setting->l_vs_luma *= 2;
		setting->l_ve_luma *= 2; // TODO: check if +1 is needed here?
		setting->r_vs_luma = 0;
		setting->r_ve_luma = 0;
		setting->r_vs_chrm = 0;
		setting->r_ve_chrm = 0;
	...
	struct blend_setting_s *setting)
{
	...
	if ((dispbuf && (dispbuf->type & VIDTYPE_MVC)) || (process_3d_type & MODE_3D_FP)) {
	...
	if ((cur_frame_par->VPP_vd_end_lines_ - cur_frame_par->VPP_vd_start_lines_ + 1) > VPP_PREBLEND_VD_V_END_LIMIT) {
		setting->preblend_v_start = 0;
		setting->preblend_v_end = VPP_PREBLEND_VD_V_END_LIMIT - 1;
	} else {
		if (legacy_vpp) {
			setting->preblend_v_start = cur_frame_par->VPP_vd_start_lines_;
			setting->preblend_v_end = cur_frame_par->VPP_vd_end_lines_;
		} else {
			setting->preblend_v_start = 0;
			setting->preblend_v_end = cur_frame_par->VPP_vd_end_lines_ - cur_frame_par->VPP_vd_start_lines_;
		}
	}
	...
	if (setting->id == 0) {
		if ((process_3d_type & MODE_3D_OUT_TB) && !is_need_mvc_to_tab_output()) {
			...
			setting->preblend_h_start = cur_frame_par->VPP_hd_start_lines_;
	...
	const struct vinfo_s *vinfo)
{
	...
	if (vd_layer[0].enable_3d_mode == mode_3d_mvc_enable || vd_layer[0].enable_3d_mode == mode_3d_enable)
	...
	if (mode & COMPOSE_MODE_DV) {
		vd_layer[1].layer_alpha =
			(vpp_misc_set >> VPP_VD2_ALPHA_BIT) & 0x1ff;
	...
	if (!(mode & COMPOSE_MODE_3D) && !(mode & COMPOSE_MODE_DV) &&
	...
	if (!legacy_vpp) {
		...
		if ((vpp_misc_set != vpp_misc_save) || force_flush) {
			...
			if (vd_layer[1].enabled && (mode & COMPOSE_MODE_DV))
...
static bool is_enable_3d_sbs_or_tab(void) {
	return (process_3d_type & MODE_3D_ENABLE) &&
		((process_3d_type & MODE_3D_OUT_LR) || (process_3d_type & MODE_3D_OUT_TB));
}
	...
	struct disp_info_s *disp_info)
{
	...
	if (((vf->type & VIDTYPE_MVC) || (process_3d_type & MODE_3D_FP)) && (layer_id == 0))
		is_mvc = true;
	...
	if (update_mif) {
		...
		if (is_mvc && cur_frame_par && (cur_frame_par->vpp_2pic_mode & VPP_PIC1_FIRST)) {
			// 3D (MVC) enabled, right eye first
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_canvas0, layer->disp_canvas[cur_canvas_id][1]);
			VSYNC_WR_MPEG_REG(vd_layer[1].vd_mif_reg.vd_if0_canvas0, layer->disp_canvas[cur_canvas_id][0]);
		} else {
			// either 2D enabled, or 3D + left eye first
			VSYNC_WR_MPEG_REG(vd_mif_reg->vd_if0_canvas0, layer->disp_canvas[cur_canvas_id][0]);

			if (is_mvc) {
				VSYNC_WR_MPEG_REG(vd_layer[1].vd_mif_reg.vd_if0_canvas0, layer->disp_canvas[cur_canvas_id][1]);
			}
		}
		...
		} else if (cur_frame_par && disp_info && is_enable_3d_sbs_or_tab() && !is_mvc) {
			VSYNC_WR_MPEG_REG(vd_layer[1].vd_mif_reg.vd_if0_canvas0, layer->disp_canvas[cur_canvas_id][0]);
			VSYNC_WR_MPEG_REG(vd_layer[1].vd_mif_reg.vd_if0_canvas1, layer->disp_canvas[cur_canvas_id][0]);
		}
#ifdef CONFIG_AMLOGIC_MEDIA_VSYNC_RDMA
	...
	const struct vinfo_s *vinfo)
{
	...
	if (((vf->type & VIDTYPE_MVC) || is_enable_3d_sbs_or_tab() || (process_3d_type & MODE_3D_FP)) && !is_enable_3d_to_2d() && (layer_id == 0))
		is_mvc = true;
	...
	if (((vf->type & VIDTYPE_NO_VIDEO_ENABLE) == 0) &&
		...
		if (frame_changed && (vf->type & VIDTYPE_MVC))
			enable_layer = true;
...
#define __DUMP_MIF_REG(l, a) { \
	u32 val = VSYNC_RD_MPEG_REG(vd_layer[l].vd_mif_reg.a); \
	u16 v1 = (u16)((val>>16)&0xffff); \
	u16 v2 = (u16)(val&0xffff); \
	pr_info("VD%d: " #a ": 0x%04x|%04x\n", l+1, v1, v2); \
}

#define __DUMP_VPP_REG(a) { \
	u32 val = VSYNC_RD_MPEG_REG(a + cur_dev->vpp_off); \
	u16 v1 = (u16)((val>>16)&0xffff); \
	u16 v2 = (u16)(val&0xffff); \
	pr_info(#a ": 0x%04x|%04x\n", v1, v2); \
}

void dump_some_registers(void)
{
	int l;

	for (l = 0; l < 2; l++) {
		__DUMP_MIF_REG(l, vd_if0_canvas0);
		__DUMP_MIF_REG(l, vd_if0_canvas1);
		__DUMP_MIF_REG(l, vd_if0_luma_x0);
		__DUMP_MIF_REG(l, vd_if0_luma_y0);
		__DUMP_MIF_REG(l, vd_if0_chroma_x0);
		__DUMP_MIF_REG(l, vd_if0_chroma_y0);
		__DUMP_MIF_REG(l, vd_if0_luma_x1);
		__DUMP_MIF_REG(l, vd_if0_luma_y1);
		__DUMP_MIF_REG(l, vd_if0_chroma_x1);
		__DUMP_MIF_REG(l, vd_if0_chroma_y1);
		__DUMP_MIF_REG(l, vd_if0_rpt_loop);
		__DUMP_MIF_REG(l, vd_if0_luma0_rpt_pat);
		__DUMP_MIF_REG(l, vd_if0_chroma0_rpt_pat);
		__DUMP_MIF_REG(l, vd_if0_luma1_rpt_pat);
		__DUMP_MIF_REG(l, vd_if0_chroma1_rpt_pat);
		__DUMP_MIF_REG(l, vd_if0_luma_psel);
		__DUMP_MIF_REG(l, vd_if0_chroma_psel);
		__DUMP_MIF_REG(l, vd_if0_luma_fifo_size);

		__DUMP_MIF_REG(l, vd_if0_gen_reg);
		__DUMP_MIF_REG(l, vd_if0_gen_reg2);
		__DUMP_MIF_REG(l, vd_if0_gen_reg3);
		__DUMP_MIF_REG(l, viu_vd_fmt_ctrl);
		__DUMP_MIF_REG(l, viu_vd_fmt_w);
	}

	// ---

	__DUMP_VPP_REG(VPP_HSC_REGION12_STARTP);
	__DUMP_VPP_REG(VPP_HSC_REGION34_STARTP);
	__DUMP_VPP_REG(VPP_HSC_REGION4_ENDP);
	__DUMP_VPP_REG(VPP_HSC_REGION1_PHASE_SLOPE);
	__DUMP_VPP_REG(VPP_HSC_REGION3_PHASE_SLOPE);
	__DUMP_VPP_REG(VPP_HSC_START_PHASE_STEP);

	__DUMP_VPP_REG(VPP_VSC_REGION12_STARTP);
	__DUMP_VPP_REG(VPP_VSC_REGION34_STARTP);
	__DUMP_VPP_REG(VPP_VSC_REGION4_ENDP);
	__DUMP_VPP_REG(VPP_VSC_START_PHASE_STEP);

	__DUMP_VPP_REG(VPP_SCALE_COEF);
	__DUMP_VPP_REG(VPP_SCALE_COEF_IDX);

	__DUMP_VPP_REG(VPP_SC_MISC);

	__DUMP_VPP_REG(VPP_LINE_IN_LENGTH);	// VD1 scaler input hsize
	__DUMP_VPP_REG(VPP_PIC_IN_HEIGHT);	// VD1 scaler input vsize
	__DUMP_VPP_REG(VPP_OUT_H_V_SIZE);	// VD1 scaler out hsize/vsize

	// ---

	__DUMP_VPP_REG(VD2_HSC_REGION12_STARTP);
	__DUMP_VPP_REG(VD2_HSC_REGION34_STARTP);
	__DUMP_VPP_REG(VD2_HSC_REGION4_ENDP);
	__DUMP_VPP_REG(VD2_HSC_START_PHASE_STEP);
	__DUMP_VPP_REG(VD2_HSC_REGION1_PHASE_SLOPE);
	__DUMP_VPP_REG(VD2_HSC_REGION3_PHASE_SLOPE);

	__DUMP_VPP_REG(VD2_VSC_REGION12_STARTP);
	__DUMP_VPP_REG(VD2_VSC_REGION34_STARTP);
	__DUMP_VPP_REG(VD2_VSC_REGION4_ENDP);
	__DUMP_VPP_REG(VD2_VSC_START_PHASE_STEP);

	if (glayer_info[1].pps_support) {
		__DUMP_VPP_REG(VD2_SC_MISC);
	}

	__DUMP_VPP_REG(VPP_VD2_HDR_IN_SIZE);	// VD2 input vsize/hsize

	// ---

	__DUMP_VPP_REG(VPP_IN_H_V_SIZE);	// not defined for SC2
	__DUMP_VPP_REG(VPP_MISC);

	__DUMP_VPP_REG(VD1_BLEND_SRC_CTRL);
	__DUMP_VPP_REG(VD2_BLEND_SRC_CTRL);
	__DUMP_VPP_REG(OSD1_BLEND_SRC_CTRL);
	__DUMP_VPP_REG(OSD2_BLEND_SRC_CTRL);
	...
	__DUMP_VPP_REG(VPP_PREBLEND_VD1_H_START_END);
	__DUMP_VPP_REG(VPP_PREBLEND_VD1_V_START_END);
	__DUMP_VPP_REG(VPP_PREBLEND_H_SIZE);
	__DUMP_VPP_REG(VPP_BLEND_VD2_H_START_END);
	__DUMP_VPP_REG(VPP_BLEND_VD2_V_START_END);
	__DUMP_VPP_REG(VPP_POSTBLEND_VD1_H_START_END);
	__DUMP_VPP_REG(VPP_POSTBLEND_VD1_V_START_END);
	__DUMP_VPP_REG(VPP_POSTBLEND_H_SIZE);

	__DUMP_VPP_REG(VPP_SRSHARP0_CTRL);
	__DUMP_VPP_REG(VPP_SRSHARP1_CTRL);
	__DUMP_VPP_REG(SRSHARP0_SHARP_HVSIZE);
	__DUMP_VPP_REG(SRSHARP1_SHARP_HVSIZE);
}

#undef __DUMP_VPP_REG
#undef __DUMP_MIF_REG
