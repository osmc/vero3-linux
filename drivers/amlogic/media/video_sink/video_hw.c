	...
	struct mif_pos_s *setting)
{
...
#endif
	setting->l_hs_chrm = setting->l_hs_luma >> 1;
	setting->l_he_chrm = ((setting->l_he_luma + 1) >> 1) - 1;
	setting->r_hs_chrm = setting->r_hs_luma >> 1;
	setting->r_he_chrm = ((setting->r_he_luma + 1) >> 1) - 1;
	setting->l_vs_chrm = setting->l_vs_luma >> 1;
	setting->l_ve_chrm = ((setting->l_ve_luma + 1) >> 1) - 1;
	setting->r_vs_chrm = setting->r_vs_luma >> 1;
	setting->r_ve_chrm = ((setting->r_ve_luma + 1) >> 1) - 1;
	...
	if ((setting->id == 0) && (dispbuf->type & VIDTYPE_MVC) && !is_enable_3d_to_2d() && !framepacking_support) {
		...
		setting->l_vs_luma *= 2;
		setting->l_ve_luma = (setting->l_ve_luma + 1) * 2 - 1;
		setting->r_vs_luma = 0;
		setting->r_ve_luma = 0;
		setting->r_vs_chrm = 0;
		setting->r_ve_chrm = 0;
	...
	const struct vinfo_s *vinfo)
{
	...
	if (((vf->type & VIDTYPE_MVC) || is_enable_3d_sbs_or_tab()) && !is_enable_3d_to_2d() && (layer_id == 0))
		is_mvc = true;
