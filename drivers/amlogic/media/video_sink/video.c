...
static DEFINE_SPINLOCK(vidcap_lock);
#endif
...
{
	struct vframe_s *my_vf = get_dispbuf(0);

	if (my_vf == NULL) {
		return -1;
	}

	atomic_inc(&my_vf->use_cnt);
	if (atomic_read(&my_vf->use_cnt) <= 1) {
		// too late, take next frame
		atomic_dec(&my_vf->use_cnt);
		return -1;
	}
...
int ext_register_end_frame_callback(struct amvideocap_req *req)
{
	...
	spin_lock_irqsave(&vidcap_lock, flags);

	if (capture_frame_req && req) {
		// we can't overwrite an active request with another one => abort
		ret = -1;
	} else {
		capture_frame_req = req;
	}

	spin_unlock_irqrestore(&vidcap_lock, flags);
...
{
	struct vframe_s *my_vf = cur_dispbuf;
	struct amvideocap_req *req;
	unsigned long flags;

	if (my_vf && my_vf->duration > 0
			&& (96000 / my_vf->duration) > 30 && (my_vf->omx_index & 1)) {
		/* if the rate is greater than 30 fps we'll skip every 2nd frame */
		return 0;
	}

	spin_lock_irqsave(&vidcap_lock, flags);
	req = capture_frame_req;
	spin_unlock_irqrestore(&vidcap_lock, flags);
	...
	if (req && req->callback) {
		...
		if (!ext_get_cur_video_frame(&vf, &index)) {
			req->callback(req->data, vf, index);
			ext_register_end_frame_callback(NULL);
		}
	}
	...
	struct vframe_s *vf1, int line)
{
	...
	if (((last_process_3d_type & MODE_3D_ENABLE) && !(process_3d_type & MODE_3D_ENABLE))
		|| (!(last_process_3d_type & MODE_3D_TO_2D_MASK) && (process_3d_type & MODE_3D_TO_2D_MASK))) {
		need_disable_vd2 = true;
	}
...
static s32 primary_render_frame(struct video_layer_s *layer)
{
	...
	struct vpp_frame_par_s local_vd2_frame_par;
	...
	local_vd2_pps.frame_par = &local_vd2_frame_par;
	...
	if (dispbuf) {
		...
		if ((dispbuf->type & VIDTYPE_INTERLACE) &&
			...
			zoom_start_y /= 2;
			zoom_end_y = ((zoom_end_y + 1) >> 1) - 1;
		} else if (dispbuf->type & VIDTYPE_MVC && !is_need_mvc_to_sbs_output() && !is_enable_3d_to_2d()) {
	...
	if ((dispbuf && (dispbuf->type & VIDTYPE_MVC)) || last_mode_3d) {
		/* we need to enable the vertical scaler if 3d to 2d conversion is needed or if the stream is interlaced */
		layer->sc_setting.sc_v_enable = is_enable_3d_to_2d() || (dispbuf && (dispbuf->type & VIDTYPE_INTERLACE));
...
#endif
{
	...
	while (vf && (vf->flag & VFRAME_FLAG_DROP_FRAME)) {
		if (is_dovi_frame(vf)) {
			if (dolby_vision_drop_frame() == 1) {
				if (debug_flag & DEBUG_FLAG_PRINT_DROP_FRAME) {
					pr_info("#line %d: DV drop frame failed\n", __LINE__);
				}
				break;
			}
		} else {
	...
	while (vf && !video_suspend) {
		...
		if (vpts_expire(cur_dispbuf, vf, toggle_cnt) || show_nosync) {
			...
			if (vd2_path_id == VFM_PATH_AMVIDEO)
				iret2 = amvecm_on_vs(
					...
					0,
					VD2_PATH);
			else if (0) //(vd2_path_id == VFM_PATH_DEF)
				iret2 = amvecm_on_vs(
					(cur_dispbuf2 != &vf_local)
					? cur_dispbuf2 : NULL,
					vf, CSC_FLAG_CHECK_OUTPUT,
					0,
					0,
			...
			force_blackout = 0;
			if (vf) {
				if (last_mode_3d != process_3d_type) {
					last_mode_3d = process_3d_type;
					mode_3d_changed = 1;
				}
		...
		// Invalidate the peek'd frame (if any) and leave the loop. That
		// way the vsync ISR would not skip the next frame if it's too
		// early for that ongoing vsync event. This is needed if Kodi
		// is not running in sync with the vsync IRQ.
		vf = NULL;
	}
	if (video_suspend)
		goto exit;
...
static void video_vf_unreg_provider(void)
{
	...
	if (cur_dispbuf2 && (blackout | force_blackout)) {
		need_disable_vd2 = true;
	}
...
void *get_sei_from_src_fmt(struct vframe_s *vf, u32 *sei_size)
{
	if (!vf || !sei_size)
		return NULL;
//pr_info("GFH %s magic code 0x%08x\n", __func__, vf->src_fmt.sei_magic_code);
...
int _video_set_disable(u32 val)
{
	...
	if (layer->disable_video != VIDEO_DISABLE_NONE) {
		...
		if (layer->dispbuf && layer->dispbuf->type & VIDTYPE_MVC) {
			// for MVC turn off all layers
			layer_id = 0xff;
		}
		...
		if (layer->dispbuf &&
			...
			if (layer->dispbuf && layer->dispbuf->type & VIDTYPE_MVC) {
				// for MVC turn on all layers
				layer_id = 0xff;
			}

			safe_switch_videolayer(layer_id, true, true);
			layer->property_changed = true;
		}
	}
	return 0;
...
{
	int cur_global_output = vd_layer[index].global_output;
	if (index == 0) {
		if (val != 0)
			vd_layer[0].global_output = 1;
		else
	...
	if (cur_global_output != vd_layer[index].global_output)
		pr_info("VID: VD%d set global output to %d, was %d\n",
			index + 1, (val != 0) ? 1 : 0, cur_global_output);
}
				     ...
				     struct class_attribute *attr, char *buf)
{
	...
	if (get_cpu_type() <= MESON_CPU_MAJOR_ID_GXTVBB) {
		val = (READ_VCBUS_REG(VPP_VADJ1_Y + cur_dev->vpp_off) >> 8) & 0x1ff;
		val = (val << 23) >> 23;

		return sprintf(buf, "%d\n", val << 2);
	} else if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) {
		val = (READ_VCBUS_REG(VPP_VADJ1_Y_2 + cur_dev->vpp_off) >> 8) & 0x7ff;
		val = (val << 21) >> 21;

		return sprintf(buf, "%d\n", val);
	}
	val = (READ_VCBUS_REG(VPP_VADJ1_Y + cur_dev->vpp_off) >> 8) & 0x3ff;
	val = (val << 22) >> 22;
				      ...
				      const char *buf, size_t count)
{
	...
	if ((r < 0) || (val < -1024) || (val > 1023))
		return -EINVAL;
	if (get_cpu_type() <= MESON_CPU_MAJOR_ID_GXTVBB) {
		WRITE_VCBUS_REG_BITS(VPP_VADJ1_Y +
			cur_dev->vpp_off, val >> 2, 8, 9);
		WRITE_VCBUS_REG_BITS(VPP_VADJ_CTRL + cur_dev->vpp_off, VPP_VADJ1_EN, 0, 1);
	} else if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) {
		WRITE_VCBUS_REG_BITS(VPP_VADJ1_Y_2 + cur_dev->vpp_off, val, 8, 11);
		WRITE_VCBUS_REG_BITS(VPP_VADJ1_MISC, VPP_VADJ1_EN, 0, 1);
	} else {
		WRITE_VCBUS_REG_BITS(VPP_VADJ1_Y +
...
static ssize_t video_contrast_show(struct class *cla,
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A)
		return sprintf(buf, "%d\n",
		(int) (READ_VCBUS_REG(VPP_VADJ1_Y_2 + cur_dev->vpp_off) &
		0xff) - 0x80);
	else
		return sprintf(buf, "%d\n",
				    ...
				    const char *buf, size_t count)
{
	...
	if ((r != 1) || (val < -127) || (val > 127))
		return -EINVAL;
	amvecm_set_contrast1(val);

	vd1_contrast = (val << 3);
//	pr_info("GFH %s contrast %d vd1_contrast %d\n", __func__, val, vd1_contrast);
...
ssize_t dump_vreg_show(struct class *cla,
			struct class_attribute *attr, char *buf)
{
#ifdef CONFIG_DEBUG_OSMC
	dump_some_registers();
#endif
