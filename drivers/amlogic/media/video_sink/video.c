...
//	*canvas_index = my_vf->canvas0Addr;
	*vf = my_vf;
...
#ifdef CONFIG_AMLOGIC_MEDIA_DEINTERLACE
		if (is_di_on() || is_di_post_link_on()) {
			/* check mif enable status, disable post di */
			VSYNC_WR_MPEG_REG(DI_POST_CTRL, 0x3 << 30);
			VSYNC_WR_MPEG_REG(
				DI_POST_SIZE,
...
static s32 primary_render_frame(struct video_layer_s *layer)
{
	...
	if (layer->dispbuf) {
		...
		if ((layer->dispbuf->type & VIDTYPE_INTERLACE) &&
			...
			zoom_start_y /= 2;
			zoom_end_y = ((zoom_end_y + 1) >> 1) - 1;
		} else if ((layer->dispbuf->type & VIDTYPE_MVC) && !is_need_mvc_to_sbs_output()) {
				      ...
				      const char *buf, size_t count)
{
	...
	if (get_cpu_type() <= MESON_CPU_MAJOR_ID_GXTVBB) {
		...
		WRITE_VCBUS_REG_BITS(VPP_VADJ1_Y_2, val, 8, 11);
...
ssize_t dump_reg_show(struct class *cla,
			struct class_attribute *attr, char *buf)
{
	dump_some_registers();
