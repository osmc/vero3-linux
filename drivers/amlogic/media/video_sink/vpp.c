	...
	struct vpp_frame_par_s *next_frame_par, struct vframe_s *vf)
{
	...
	if (!(process_3d_type & MODE_3D_FP) && height_in > src_height_max)
		next_frame_par->vscale_skip_count++;
...
RESTART:
...
#endif
	{
		...
		next_frame_par->VPP_hd_end_lines_ = (temp <= w_in ? temp : w_in) - 1;
	}
...
{
	if (*vpp_3d_mode != VPP_3D_MODE_NULL) {
	...
	if ((process_3d_type & MODE_3D_AUTO) ||
		...
		if (process_3d_type & MODE_3D_TO_2D_MASK) {
			...
			*src_height = frame_height;
			...
			if (process_3d_type & MODE_3D_FULL_LRTB) {
				*src_width = frame_width >> 1;
			} else {
				*src_width = frame_width;
			}
			*src_height = (frame_height << 1) + framepacking_blank;
			next_frame_par->vpp_2pic_mode = 1;
		} else {
			*src_width = frame_width >> 1;
		...
		if (process_3d_type & MODE_3D_TO_2D_MASK) {
			...
			*src_height = frame_height;
			...
			if (process_3d_type & MODE_3D_FULL_LRTB) {
				*src_width = frame_width;
				*src_height = frame_height + framepacking_blank;
			} else {
				*src_width = frame_width;
				*src_height = frame_height;
			}
...
RESTART:
...
#endif
	{
		...
		temp = next_frame_par->VPP_hd_start_lines_ + (temp_width * ratio_x >> 18);
		next_frame_par->VPP_hd_end_lines_ = (temp <= w_in ? temp : w_in) - 1;
	}
