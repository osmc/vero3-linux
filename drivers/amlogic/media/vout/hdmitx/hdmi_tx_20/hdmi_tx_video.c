...
void hdmitx_output_rgb(int set_rgb);
...
void hdmitx_output_rgb(int set_rgb)
{
	hdmi_output_rgb = (unsigned char) set_rgb;
}
...
static int is_philips_2009(struct rx_cap *pRXCap)
{
	if ((strncmp(pRXCap->IDManufacturerName, "PHL", strlen(pRXCap->IDManufacturerName)) == 0) && \
		...
		pr_info(SYS, "This is a 2009 Philips TV which misreports colour caps");
		return 1;
	}
	else
		return 0;
...
int hdmitx_set_display(struct hdmitx_dev *hdev, enum hdmi_vic VideoCode)
{
	...
	pr_info(VID "params found at 0x%08x\n", param);
	if (param) {
		...
		pr_info(VID "cur_param = param at 0x%08x colourdepth %d colourspace %d\n",
					hdev->cur_video_param, hdev->cur_video_param->color_depth * 2, param->color);
		param->color = hdev->para->cs;
		if (hdmi_output_rgb == 1 || is_philips_2009(&hdev->rxcap) ||
			hdev->para->cs == COLORSPACE_RGB444) {
			param->color = COLORSPACE_RGB444;
			pr_info("Set RGB - para->cs %d output_rgb %d", hdev->para->cs, hdmi_output_rgb);
		} else if (!is_dv_output()) {
			param->color = param->color_prefer;
			/* HDMI CT 7-24 Pixel Encoding
			 * YCbCr to YCbCr Sink
			 */
			//pr_info("GFH native mode 0x%02x", hdev->rxcap.native_Mode);
		...
		pr_info(VID "pixel freq %d, tmds clock %d, MaxClock1 %d, HF_IEEEOUI 0x%06x, MaxClock2 %d\n",
		...
		if (hdev->para->tmds_clk * ((int) param->color_depth) / ((int) COLORDEPTH_24B) >
			max_tmds_clock * (param->color == COLORSPACE_YUV420 ? 2 : 1)) {
			if (hdev->para->tmds_clk <= max_tmds_clock) {
				...
				if (hdev->rxcap.native_Mode & 0x10) {
					...
					pr_info("No support for 10 bits or YCC422, setting 8-bit\n");
		...
		if ((param->color == COLORSPACE_YUV422 || param->color == COLORSPACE_YUV420)
			...
			/* this can only happen with 4k50/60hz when no HF-IEEEOUI indicating a suitable tmds clock */
			pr_info(VID "Colourspace or bitdepth too high for 50/60Hz, setting YCC420, 8 bits");
			param->color_depth = COLORDEPTH_24B;
			param->color = COLORSPACE_YUV420;
		}
...
*/
		...
		if (VideoCode >= HDMITX_VESA_OFFSET) {
			if (hdev->para->cs == COLORSPACE_RGB444)
				hdev->cur_video_param->color_depth = COLORDEPTH_24B;
			else
				hdev->para->cs = COLORSPACE_YUV422;
			pr_info("hdmitx: VESA only support 8-bit timings\n");
		}
