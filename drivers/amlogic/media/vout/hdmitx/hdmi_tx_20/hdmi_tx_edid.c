...
static void store_vesa_idx(struct rx_cap *prxcap, enum hdmi_vic vesa_timing)
{
	...
	for (i = 0; i < VESA_MAX_TIMING; i++) {
		if (!prxcap->vesa_timing[i] && vesa_timing > HDMITX_VESA_OFFSET) {
			prxcap->vesa_timing[i] = vesa_timing;
			pr_info("hdmitx: vic %d added to vesa modes\n", vesa_timing);
			break;
		}
	...
	unsigned char *buff)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (i == 0)
			prxcap->native_VIC = prxcap->dtd[prxcap->dtd_idx-1].vic;
		dtd_base += 0x12;
	}
}
	...
	unsigned char *buff)
{
	...
	while (BlockAddr < VSpecificBoundary) {
		...
		if ((buff[BlockAddr] & 0xE0) == VENDOR_TAG &&
			(buff[BlockAddr + 1] == GET_OUI_BYTE0(HDMI_IEEEOUI)) &&
	...
	unsigned char *blockbuf)
{
	...
	char dummy_hdr10p_vsvdb[] = {0xe5,0x01,0x8b,0x84,0x90,0x011};
 ...
 */
	...
	if (!has_hdr_static && get_force_hdr()){
		dummy_hdr_static[2] = ((get_force_hdr() & 0x3) << 2) | 1;
		edid_parsedrmsb(prxcap,dummy_hdr_static);
		pr_info("EDID: Forcing %s%s caps\n", 
			get_force_hdr() & 0x1 ? "HDR10" : "",
			get_force_hdr() == 0x2 ? "HLG" :
			get_force_hdr() == 0x3 ? " and HLG" : ""); 
	}
	if ((get_force_hdr() & 0x5) == 0x5){
		Edid_ParsingVendSpec(hdev, prxcap, dummy_hdr10p_vsvdb);
		pr_info("EDID: Forcing HDR10+ caps");
	}
...
/* check EDID strictly */
/* no longer used - fails to accept DVI EDIDs*/
#if 0
 ...
 */
	...
	if (para) {
		...
		if (para->vic < HDMITX_VESA_OFFSET)
			...
			if (para->vic == HDMIV_1366x768p60hz)
				store_vesa_idx(prxcap, HDMIV_1360x768p60hz);
			store_vesa_idx(prxcap, para->vic);
		}
	} else if (t->pixel_clock > 0)
		dump_dtd_info(t);
}
...
int hdmitx_edid_parse(struct hdmitx_dev *hdmitx_device)
{
	...
	/* HF-EEODB */
	if (BlockCount == 1) {
		for (i = 132; i < (128 + EDID_buf[130]); i++) {
			if (EDID_buf[i] == 0xe2 &&
				EDID_buf[i + 1] == 0x78) {
				BlockCount = EDID_buf[i + 2];
				break;
			}
			else
				i += (EDID_buf[i] & 0x1F);
	...
	pr_info (EDID "extension block count %d\n", BlockCount);
 ...
 */
	...
	for (i = 0; i < 4; i++) {
		if ((EDID_buf[idx[i]]) && (EDID_buf[idx[i] + 1])) {
			...
			if (i == 0) {
				Edid_Descriptor_PMT(prxcap, &t,
						    &EDID_buf[idx[i]]);
				/* prefer this pixel clock to any lower in range block */
				maxPixelClock = (t.tmds_clk + 1000 - 1) / 1000;
			}
			if (i == 1)
		...
		case TAG_RANGE_LIMITS:
			if (EDID_buf[idx[i]+9] > maxPixelClock)
	...
	if (prxcap->Max_TMDS_Clock1 < 15){
		if (maxPixelClock < 8){
			pr_info(EDID "No pixel clock value found - setting 75MHz");
...
static struct dispmode_vic dispmode_vic_tab[] = {
	...
	{"1920x1200p50hz", HDMIV_1920x1200p50hz},
	{"1920x1200p48hz", HDMIV_1920x1200p48hz},
	{"1920x1200p30hz", HDMIV_1920x1200p30hz},
	{"1920x1200p25hz", HDMIV_1920x1200p25hz},
	{"1920x1200p24hz", HDMIV_1920x1200p24hz},
	{"2048x1080p24hz", HDMIV_2048x1080p24hz},
	{"2160x1200p90hz", HDMIV_2160x1200p90hz},
	{"2560x1080p60hz_v", HDMIV_2560x1080p60hz},
	{"2560x1080p75hz", HDMIV_2560x1080p75hz},
	...
	const char *disp_mode, char force_flag)
{
	...
	if (vic != HDMI_Unknown) {
		if (force_flag == 0 && vic != HDMIV_640x480p60hz) {
			for (j = 0 ; j < prxcap->VIC_count ; j++) {
				if (prxcap->VIC[j] == vic)
					break;
			}
...
static void hdmitx_edid_blk_print(unsigned char *blk, unsigned int blk_idx)
{
	...
	for (i = 0, pos = 0; i < 128; i++) {
		...
		if (((i+1) & 0xf) == 0)    /* print 16 bytes a line */
			sprintf(tmp_buf + pos - 1, "\n");
	}
