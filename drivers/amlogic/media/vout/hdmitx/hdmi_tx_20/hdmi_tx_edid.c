...
static void calc_timing(unsigned char *data, struct vesa_standard_timing *t)
{
	...
	pr_info("Looking up %d x %d x %dHz\n", t->hactive, t->vactive, t->hsync);
	...
	for (offset = 4 ; offset < end ; ) {
		...
		case HDMI_EDID_BLOCK_TYPE_VIDEO:
			...
			for (i = 0 ; i < count ; i++) {
				...
				else if (blockbuf[offset + i] & 0x80) {
					if (prxcap->native_VIC != VIC)
						pr_info(EDID 
							"Native VIC in SVD not same as DTD[0] (%d)\n",
							prxcap->native_VIC);
					prxcap->native_VIC = VIC;
				}
				prxcap->VIC[prxcap->VIC_count] = VIC;
				prxcap->VIC_count++;
				pr_info(EDID "Added VIC %d from SVD %d\n", VIC, i);
			}
 ...
 */
	...
	if (!has_hdr_static && get_force_hdr()){
		Edid_ParsingDRMStaticBlock(prxcap,dummy_hdr_static);
	...
	i = 0;
	while (blockbuf[blockbuf[2] + i * 18 + 1] != 0)
 ...
 */
	...
	if (para) {
		...
		prxcap->native_VIC = prxcap->dtd[0].vic;
...
int hdmitx_edid_parse(struct hdmitx_dev *hdmitx_device)
{
	...
	prxcap->native_VIC = 0xff;
	for (i = 0; i < 4; i++)
		Edid_DTD_parsing(prxcap, &EDID_buf[0x36 + i * 18]);
	...
	/* Add vesa modes to vesa_timing from dtd[].
	 * If there is only one block, add CTA modes as well.
	 */
	
	 for (i = 0; i < prxcap->dtd_idx; i++){
		 if (prxcap->dtd[i].vic >= HDMITX_VESA_OFFSET)
			 store_vesa_idx(prxcap, prxcap->dtd[i].vic);
		 else if (BlockCount == 0)
			 prxcap->VIC[prxcap->VIC_count++] = prxcap->dtd[i].vic;
	 } 
/*
...
enum hdmi_vic hdmitx_edid_vic_tab_map_vic(const char *disp_mode)
{
	...
	if (vic == HDMI_Unknown)
		pr_info(EDID "not find mapped vic %s\n", disp_mode);
	...
	int buffer_len)
{
	...
	if (prxcap->hdr10plus_info.ieeeoui == 0x90848b)
		pos += snprintf(buffer+pos, buffer_len-pos,
			"  HDR10plus %d", prxcap->hdr10plus_info.application_version);
