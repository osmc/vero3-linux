...
//protos
unsigned int hdmitx_rd_reg_osmc_legacy(unsigned int addr);
void hdmitx_wr_reg_osmc_legacy(unsigned int addr, unsigned int data);
...
unsigned int hdmitx_rd_reg(unsigned int addr)
{
	...
	if (hdev->chip_type == MESON_CPU_ID_GXL)
		data = hdmitx_rd_reg_osmc_legacy(addr);
	else
...
void hdmitx_wr_reg(unsigned int addr, unsigned int data)
{
	...
	if (hdev->chip_type == MESON_CPU_ID_GXL)
		hdmitx_wr_reg_osmc_legacy(addr, data);
	else
		hdmitx_wr_reg_normal(addr, data);
}
