...
void set_sc2_hpll_clk_out(unsigned int frac_rate, unsigned int clk)
{
	...
	case 4032000:
		...
		break;
	default:
		if (clk > 5940000 || clk < 2970000)
			pr_info("error hpll clk: %d\n", clk);
		else {
			int nm;
			uint32_t div_frac;
			clk -= clk * frac_rate / 1000;
			nm = clk  / 24000 + 1024;
			pr_info("mod clk 24k %i", (clk/1000) % 24);
			div_frac = clk;
			pr_info("clk %i", div_frac);
			div_frac -= (nm-1024)*24000;
			pr_info("remainder %i", div_frac);
			div_frac *= 131072;
			pr_info("times 131k %i", div_frac);
			div_frac /= 24000;
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL0, (0x3b000000 | nm));
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL1, div_frac);
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL2, 0x00000000);
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL3, 0x0a691c00);
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL4, 0x33771290);
			hd_write_reg(P_ANACTRL_HDMIPLL_CTRL5, 0x39270008);
			...
			hd_set_reg_bits(P_ANACTRL_HDMIPLL_CTRL0, 0x0, 29, 1);
			WAIT_FOR_PLL_LOCKED(P_ANACTRL_HDMIPLL_CTRL0);
			pr_info("HPLL: (calculated) 0x%x 0x%x 0x%x\n", hd_read_reg(P_ANACTRL_HDMIPLL_CTRL0),
				hd_read_reg(P_ANACTRL_HDMIPLL_CTRL1), div_frac);
		break;
		}
	}
}
