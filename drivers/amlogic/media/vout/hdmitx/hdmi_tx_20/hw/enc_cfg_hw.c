...
static const struct reg_s tvregs_vesa_2560x1440p60hz[] = {
	...
	{P_ENCP_VIDEO_MODE, 0x4040,},
	{P_ENCP_VIDEO_MODE_ADV, 0x18,},
	{P_ENCP_VIDEO_MAX_PXCNT, 0,}, // 0xA1F,},
	{P_ENCP_VIDEO_MAX_LNCNT, 0x4DC,},
	{P_ENCP_VIDEO_HAVON_BEGIN, 0x218,},
	{P_ENCP_VIDEO_HAVON_END, 0x997,},
	{P_ENCP_VIDEO_VAVON_BLINE, 0x2A,},
...
static struct reg_s tvregs_custom[] = {
	{P_ENCP_VIDEO_EN, 0,}, //6e00
	{P_ENCI_VIDEO_EN, 0,}, //6d5c
	{P_VENC_VDAC_SETTING, 0xff,}, //6df8

	{P_ENCP_VIDEO_MODE, 0x4040,}, //6e34
	{P_ENCP_VIDEO_MODE_ADV, 0x18,}, //6e38
	{P_ENCP_VIDEO_MAX_PXCNT, 0x0,}, //6e5c
	{P_ENCP_VIDEO_MAX_LNCNT, 0x0,}, //6eb8
	{P_ENCP_VIDEO_HAVON_BEGIN, 0x0,}, //6e90
	{P_ENCP_VIDEO_HAVON_END, 0x0,}, //6e8c
	{P_ENCP_VIDEO_VAVON_BLINE, 0x0,}, //6e98
	{P_ENCP_VIDEO_VAVON_ELINE, 0x0,}, //6ebc
	{P_ENCP_VIDEO_HSO_BEGIN, 0x0,}, //6e9c
	{P_ENCP_VIDEO_HSO_END, 0x0,}, //6ea0
	{P_ENCP_VIDEO_VSO_BEGIN, 0x0,}, //6ea4
	{P_ENCP_VIDEO_VSO_END, 0x0,},  //6ea8
	{P_ENCP_VIDEO_VSO_BLINE, 0x0,}, //6eac
	{P_ENCP_VIDEO_VSO_ELINE, 0x0,}, //6eb0
	{P_ENCI_VIDEO_EN, 0,}, //6d5c
	{MREG_END_MARKER, 0} //ffff
};
...
static struct vic_tvregs_set tvregsTab[] = {
	...
	{HDMIV_1920x1200p50hz, tvregs_vesa_1920x1200p60hz},
	{HDMIV_1920x1200p30hz, tvregs_vesa_1920x1200p60hz},
	{HDMIV_1920x1200p25hz, tvregs_vesa_1920x1200p60hz},
	{HDMIV_1920x1200p24hz, tvregs_vesa_1920x1200p60hz},
...
void set_vmode_enc_custom(struct hdmitx_dev *hdev);
...
void set_vmode_enc_hw(struct hdmitx_dev *hdev)
{
	...
	if (s) {
		...
		/* check if we have custom timings */
		while (s->reg != P_ENCP_VIDEO_MAX_PXCNT)
			s++;
		pr_err("s-val %u h_total %u", s->val, hdev->para->timing.h_total);
		if (s->val != hdev->para->timing.h_total - 1)
			set_vmode_enc_custom(hdev);
		else {
...
void set_vmode_enc_custom(struct hdmitx_dev *hdev)
{
#define DUMP_REG(a) pr_info("0x%04x: 0x%08x", a, hd_read_reg(a))
	struct reg_s *custom_reg = tvregs_custom;
	struct hdmi_cea_timing *t = &(hdev->para->timing);
	pr_info("Custom timing: h_total %d v_total %d", t->h_total, t->v_total);
	unsigned int value;
	while (custom_reg->reg != MREG_END_MARKER) {
		switch (custom_reg->reg) {
			case P_ENCP_VIDEO_MAX_PXCNT:
				value = t->h_total -1;
				break;
			case P_ENCP_VIDEO_MAX_LNCNT:
				value = t->v_total -1;
				break;
			case P_ENCP_VIDEO_HAVON_BEGIN:
				value = t->h_blank - t->h_front;
				break;
			case P_ENCP_VIDEO_HAVON_END:
				value = t->h_total - t->h_front - 1;
				break;
			case P_ENCP_VIDEO_VAVON_BLINE:
				value = t->v_blank - t->v_front;
				break;
			case P_ENCP_VIDEO_VAVON_ELINE:
				value = t->v_total - t->v_front - 1;
				break;
			case P_ENCP_VIDEO_HSO_BEGIN:
				value = 0;
				break;
			case P_ENCP_VIDEO_HSO_END:
				value = t->h_sync;
				break;
			case P_ENCP_VIDEO_VSO_BEGIN:
				value = 30;
				break;
			case P_ENCP_VIDEO_VSO_END:
				value = 50;
				break;
			case P_ENCP_VIDEO_VSO_BLINE:
				value = 0;
				break;
			case P_ENCP_VIDEO_VSO_ELINE:
				value = t->v_sync;
				break;
			default:
				value = custom_reg->val;
				break;
		}
		pr_info("Custom reg: 0x%04x: 0x%08x", custom_reg->reg, value);
		custom_reg->val = value;
		setreg(custom_reg);
		custom_reg++;
	}
}
