...
static struct hw_enc_clk_val_group setting_enc_clk_val_24[] = {
	...
	{{HDMIV_1920x1200p50hz,
	  HDMI_VIC_END},
		5200000, 4, 1, 1, VID_PLL_DIV_5, 2, 1, 1, -1},
	{{HDMIV_1920x1200p48hz,
	  HDMI_VIC_END},
		4992000, 4, 1, 1, VID_PLL_DIV_5, 2, 1, 1, -1},
	{{HDMIV_1920x1200p30hz,
	  HDMI_VIC_END},
		3120000, 4, 1, 1, VID_PLL_DIV_5, 2, 1, 1, -1},
	{{HDMIV_1920x1200p24hz,
	  HDMI_VIC_END},
		4992000, 4, 2, 1, VID_PLL_DIV_5, 2, 1, 1, -1},
	...
	{{HDMIV_2560x1440p60hz,
	  HDMI_VIC_END},
		4830000, 1, 2, 2, VID_PLL_DIV_5, 1, 1, 1, -1},
...
static struct hw_enc_clk_val_group setting_enc_clk_val_30[] = {
	...
	{{HDMI_VIC_FAKE,
	  HDMI_VIC_END},
		3450000, 1, 2, 1, VID_PLL_DIV_6p25, 2, 1, 1, -1},
};
...
static struct hw_enc_clk_val_group setting_enc_clk_val_36[] = {
	...
	{{HDMI_VIC_FAKE,
	  HDMI_VIC_END},
		3450000, 1, 2, 1, VID_PLL_DIV_7p5, 2, 1, 1, -1},
};
	...
	if (hdev->flag_3dfp) {
		...
		for (j = 0; j < sizeof(setting_enc_clk_val_24)
			...
			for (i = 0; ((i < GROUP_MAX) && (p_enc[j].group[i]
				!= HDMI_VIC_END)); i++) {
				if (vic == p_enc[j].group[i]) {
					/* deal with non-standard timings for VESA modes */
					if (vic >= HDMITX_VESA_OFFSET && hdev->para->cta_vesa_default) {
						pr_info("hpll_clk_out was %u\n", p_enc[j].hpll_clk_out);
						p_enc[j].hpll_clk_out = hdev->para->tmds_clk *
							p_enc[j].od1 * p_enc[j].od2 * 10;
					}
					goto next;
				}
			}
		...
		if (j == sizeof(setting_enc_clk_val_30) /
			...
			j--;
			int od1_2;
			od1_2 = 5940000 / 25 * 2 / hdev->para->timing.pixel_freq;
			if (od1_2 < 2)
				p_enc[j].od1 = p_enc[j].od2 = 1;
			else if (od1_2 < 4) {
				p_enc[j].od1 = 2;
				p_enc[j].od2 = 1;
			} else if (od1_2 < 8) {
			...
			p_enc[j].hpll_clk_out = hdev->para->timing.pixel_freq *
				p_enc[j].od1 * p_enc[j].od2 * 25 / 2;
			pr_info ("od1 %d od2 %d od3 %d\n", p_enc[j].od1, p_enc[j].od2, p_enc[j].od3);
			goto next;
		}
		...
		if (j == sizeof(setting_enc_clk_val_36) /
			...
			j--;
			int od1_2;
			od1_2 = 5940000 / 15 / hdev->para->timing.pixel_freq;
			if (od1_2 < 2)
				p_enc[j].od1 = p_enc[j].od2 = 1;
			else if (od1_2 < 4) {
				p_enc[j].od1 = 2;
