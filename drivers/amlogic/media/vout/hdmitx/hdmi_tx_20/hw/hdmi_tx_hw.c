	...
	enum hdmi_color_depth cd)
{
	...
	case COLORSPACE_YUV422:
		...
		case COLORDEPTH_36B:
			...
			break;
		case COLORDEPTH_48B:
			pr_info(HW "y422 no 48bits mode\n");
			break;
		default:
			break;
		}
...
static void hdmi_tvenc_vesa_set(struct hdmitx_vidpara *param)
{
	...
	unsigned int vsync_polarity, hsync_polarity;
	...
	vsync_polarity = vtiming->vsync_polarity;
	hsync_polarity = vtiming->hsync_polarity;
	...
	struct hdmitx_audpara *audio_param)
{
	...
	pr_info(HW "set channel status, sample rate %d\n", audio_param->sample_rate);
	for (i = 0; i < 9; i++)
	...
	struct hdmitx_audpara *audio_param)
{
	...
	switch (audio_param->type) {
	case CT_MAT:
		pr_info(HW "Audio Type: MAT(MLP)\n");
	case CT_DTS_HD_MA:
		if (audio_param->type == CT_DTS_HD_MA)
			pr_info(HW "Audio Type: DTS HD MA\n");
		/* CC: refer to stream */
	...
	/* DTS-HD-HRA */
	case CT_DTS_HD:
		pr_info(HW "Audio Type: DTS-HD HRA\n");
		hdmitx_set_reg_bits(HDMITX_DWC_FC_AUDICONF0, 1, 4, 3);
		hdmitx_wr_reg(HDMITX_DWC_FC_AUDICONF2, 0x0);
		break;
	case CT_PCM:
		pr_info(HW "Audio Type: PCM  Audio Channels: %u, Speaker layout: 0x%x\n",
				audio_param->channel_num + 1, hdev->speaker_layout);
		pr_info(HW "Audio Output Channels set to: %x:%x\n",
	...
	default:
		switch (audio_param->type) {
			case CT_DTS:
				pr_info(HW "Audio Type: DTS\n");
				break;
			case CT_AC_3:
				pr_info(HW "Audio Type: AC3\n");
				break;
			case CT_DOLBY_D:
				pr_info(HW "Audio Type: Dolby Digital +\n");
				break;
			default:
				pr_info(HW "Audio Type: #%d\n", audio_param->type);
		}
		/* CC: 2ch */
		hdmitx_set_reg_bits(HDMITX_DWC_FC_AUDICONF0, 1, 4, 3);
		hdmitx_wr_reg(HDMITX_DWC_FC_AUDICONF2, 0x0);
	...
	struct hdmitx_audpara *audio_param)
{
	...
	if (!audio_param)
		return 0;
	pr_info(HW "set audio type %d channels %d + 1\n", audio_param->type, audio_param->channel_num);
...
/* [  3] SPDIF fifo_empty_mask: 0=enable int; 1=mask int. */
	...
	if (hdev->tx_aud_src == 1) {
		if (GET_OUTCHN_MSK(hdev->aud_output_ch)) {
			pr_info(HW "hdmitx channel mask = %d\n", GET_OUTCHN_MSK(hdev->aud_output_ch));
			hdmitx_set_reg_bits(HDMITX_DWC_AUD_CONF0,
				GET_OUTCHN_MSK(hdev->aud_output_ch), 0, 4);
		}
		else {
			pr_info(HW "hdmitx no channel mask - assume stereo");
			hdmitx_set_reg_bits(HDMITX_DWC_AUD_CONF0, 0x1, 0, 4);
		}
	...
	unsigned int argv)
{
	...
	case CONF_AVI_BT2020:
		if (argv == SET_AVI_BT2020) {
			...
			//pr_info(VID "Signaling HDR content\n");
		}
...
void hdmitx_set_avi_colorimetry(struct hdmi_format_para *para)
{
	...
	default:
		if (hdev->colormetry) {
			hdmitx_set_reg_bits(HDMITX_DWC_FC_AVICONF1, 3, 6, 2);
			hdmitx_set_reg_bits(HDMITX_DWC_FC_AVICONF2, 6, 4, 3);
		} else if (para->vic < HDMITX_VESA_OFFSET) {
			/* C1C0 709 */
 ...
 */
	...
	/* For VESA modes, set VIC as 0 and underscanned */
	if (para->vic >= HDMITX_VESA_OFFSET) {
		hdmitx_wr_reg(HDMITX_DWC_FC_AVIVID, 0);
		hdmitx_set_isaformat(para->vic);
		if (hdev->hdmi_info.support_underscan_flag)
			hdmitx_set_reg_bits(HDMITX_DWC_FC_AVICONF0, 2, 4, 2);
		/* aspect ratio could  be anything */
		hdmitx_set_reg_bits(HDMITX_DWC_FC_AVICONF1, 0, 4, 2);
	}
			...
			unsigned char color_depth)
{
	...
	conv_en |= (get_range_control() & 2) ? 1 : 0;
	if (is_dv_output())
		conv_en = 0;

	if (conv_en) {
		if (output_color_format == COLORSPACE_RGB444) {
			if (get_range_control() & 2){
				/* output full range */
				csc_coeff_a1 = 0x12a1;
				csc_coeff_a2 = 0x7779;
				csc_coeff_a3 = 0x7c96;
				csc_coeff_a4 = (color_depth == COLORDEPTH_24B) ? 0x004c :
					(color_depth == COLORDEPTH_30B) ? 0x0133 :
					(color_depth == COLORDEPTH_36B) ? 0x04ce :
					(color_depth == COLORDEPTH_48B) ? 0x10ee : 0x004c;
				csc_coeff_b1 = 0x12a1;
				csc_coeff_b2 = 0x1caf;
				csc_coeff_b3 = 0x0000;
				csc_coeff_b4 = (color_depth == COLORDEPTH_24B) ? 0x7f07 :
					(color_depth == COLORDEPTH_30B) ? 0x7c1f :
					(color_depth == COLORDEPTH_36B) ? 0x707e :
					(color_depth == COLORDEPTH_48B) ? 0x6992 : 0x7f07;
				csc_coeff_c1 = 0x12a1;
				csc_coeff_c2 = 0x0000;
				csc_coeff_c3 = 0x21cc;
				csc_coeff_c4 = (color_depth == COLORDEPTH_24B) ? 0x7ede :
					(color_depth == COLORDEPTH_30B) ? 0x7b7b :
					(color_depth == COLORDEPTH_36B) ? 0x7def :
					(color_depth == COLORDEPTH_48B) ? 0x63a6 : 0x7ede;
				csc_scale = 2;
			} else {
				csc_coeff_a1 = 0x2000;
				csc_coeff_a2 = 0x715a;
				csc_coeff_a3 = 0x7a23;
				csc_coeff_a4 = (color_depth == COLORDEPTH_24B) ? 0x00a4 :
					(color_depth == COLORDEPTH_30B) ? 0x0290 :
					(color_depth == COLORDEPTH_36B) ? 0x0a40 :
					(color_depth == COLORDEPTH_48B) ? 0x10ee : 0x00a4;
				csc_coeff_b1 = 0x2000;
				csc_coeff_b2 = 0x3144;
				csc_coeff_b3 = 0x0000;
				csc_coeff_b4 = (color_depth == COLORDEPTH_24B) ? 0x7e75 :
					(color_depth == COLORDEPTH_30B) ? 0x79d7 :
					(color_depth == COLORDEPTH_36B) ? 0x675d :
					(color_depth == COLORDEPTH_48B) ? 0x6992 : 0x7e75;
				csc_coeff_c1 = 0x2000;
				csc_coeff_c2 = 0x0000;
				csc_coeff_c3 = 0x3a0d;
				csc_coeff_c4 = (color_depth == COLORDEPTH_24B) ? 0x7e2f :
					(color_depth == COLORDEPTH_30B) ? 0x78be :
					(color_depth == COLORDEPTH_36B) ? 0x62f9 :
					(color_depth == COLORDEPTH_48B) ? 0x63a6 : 0x7e2f;
				csc_scale = 1;
			}
		} else if (get_range_control() & 2) {
			/* output full range YCC */
			csc_coeff_a1 = 0x2542;
			csc_coeff_a2 = 0x0000;
			csc_coeff_a3 = 0x0000;
			csc_coeff_a4 = (color_depth == COLORDEPTH_24B) ? 0x7fda :
				(color_depth == COLORDEPTH_30B) ? 0x7f6a :
				(color_depth == COLORDEPTH_36B) ? 0x7dab :
				(color_depth == COLORDEPTH_48B) ? 0x10ee : 0x7fda;
			csc_coeff_b1 = 0x0000;
			csc_coeff_b2 = 0x246d;
			csc_coeff_b3 = 0x0000;
			csc_coeff_b4 = (color_depth == COLORDEPTH_24B) ? 0x7fdc :
				(color_depth == COLORDEPTH_30B) ? 0x7f72 :
				(color_depth == COLORDEPTH_36B) ? 0x7dc9 :
				(color_depth == COLORDEPTH_48B) ? 0x6992 : 0x7fdc;
			csc_coeff_c1 = 0x0000;
			csc_coeff_c2 = 0x0000;
			csc_coeff_c3 = 0x246d;
			csc_coeff_c4 = (color_depth == COLORDEPTH_24B) ? 0x7fdc :
				(color_depth == COLORDEPTH_30B) ? 0x7f72 :
				(color_depth == COLORDEPTH_36B) ? 0x7dc9 :
				(color_depth == COLORDEPTH_48B) ? 0x63a6 : 0x7fdc;
			csc_scale = 1;
		} else { /* input_color_format == COLORSPACE_RGB444 full range */
			csc_coeff_a1 = 0x2591;
			csc_coeff_a2 = 0x1322;
			csc_coeff_a3 = 0x074b;
			csc_coeff_a4 = 0x0000;
