...
static bool is_sink_support_dv_std(void);
...
static u32 hdr_features = 0;
/* 1 add HDR10 cap, 2 add HLG cap, 4 add HDR10+ cap */
...
static bool is_sink_support_dv_std(void){
	struct dv_info *dv_caps;
	if (&hdmitx_device.rxcap.dv_info) {
		dv_caps = &hdmitx_device.rxcap.dv_info;
		if (dv_caps->ieeeoui != DV_IEEE_OUI)
			return false;
		if ((dv_caps->ver == 2 && (dv_caps->Interface & 2))
			|| dv_caps->ver < 2)
			return true;
	}
	return false;
...
bool is_dv_output(void) {
	return is_dolby_vision_enable()
		&& is_sink_support_dv_std()
		&& get_hdr_mode() == 2;
	/* TODO provide an OFF switch */
}
EXPORT_SYMBOL(is_dv_output);
...
static int set_disp_mode_auto(void)
{
	...
	char* dv_type[] = {"Not DV", "DV-Std", "DV-LL"};
	char* colorimetry[] = {"no data","SMPTE 170M","BT709","xvYCC601","xvYCC709","sYCC601",
		"opYCC601","opRGB","BT2020c","BT2020nc","DCI-P3-D65","DCI-P3"};
	char* color;
	int co, extco;

	/* vic_ready got from IP *************** MISSING FROM 4.9.180 ****************/
	...
	if ((info == NULL) || (info->name == NULL))
		return -1;
	pr_info(VID "auto - get current mode: %s vmode_e %d\n",
		info ? info->name : "null", info ? info->mode : 0);

	/* save colourdepth of the stream */
	stream_cur_cd = COLORDEPTH_24B;
	if (hdev->para && hdev->para->cd && hdev->para->name) {
		stream_cur_cd = hdev->para->cd;
		pr_info(SYS "stream colourdepth was %d in para %p (%s)\n",stream_cur_cd * 2, hdev->para, hdev->para->name);
	} else
		pr_info(SYS "hdev->para not available");
	if (hdev->cur_video_param != NULL){
		pr_info(SYS "display colourdepth was %d in cur_param 0x%08x (VIC: %d)\n",hdev->cur_video_param->color_depth * 2,
		...
		if (hdev->cur_video_param->color_depth > COLORDEPTH_24B && hdev->para->cs != COLORSPACE_YUV422){
			...
			if (! dc_support){
				pr_warn("Bitdepth is set to %d bits but display does not support deep colour",
						hdev->cur_video_param->color_depth * 2);
				/* use YUV422 if supported but don't override RGB setting */
				if (hdev->hdmi_info.support_ycbcr422_flag == 1 && hdev->para->cs > 0)
					hdev->para->cs = COLORSPACE_YUV422;
				/* or drop to 8 bit unless forced */
				else if (strstr(hdev->fmt_attr,"bit") == NULL)
					hdev->cur_video_param->color_depth = COLORDEPTH_24B;
			}
		}
	}
	...
	if (strstr(hdev->fmt_attr,"full") || get_range_control() & 2){
               hdev->para->cr = COLORRANGE_FUL;
	}
	else if (!is_dv_output()) {
		hdev->para->cr = COLORRANGE_LIM;
	}

	/* and recover the original bitstream bitdepth */
	hdev->para->cd = stream_cur_cd;
	pr_info(SYS "Stream colourdepth is %d in para 0x%08x (%s - VIC: %d)\n",stream_cur_cd * 2, hdev->para, hdev->para->name, hdev->para->vic);
	...
	/* This bit is missing from 4.9.180 drop *****************************************/
	if ((vic_ready != HDMI_Unknown) && (vic_ready == vic) && (strstr(hdev->fmt_attr,"now") == NULL)) {
	...
	if (strstr(hdev->fmt_attr,"now") != NULL){
...
/* if vic is HDMI_Unknown, hdmitx_set_display will disable HDMI */
	...
	pr_info(VID "Video output\n\tBit depth: %d-bit, Colourspace: %s\n\tColour range RGB: %s, YCC: %s\n",
	...
	if ((co =(hdmitx_rd_reg(HDMITX_DWC_FC_AVICONF1) >> 6) & 3) < 3)
		color = colorimetry[co];
	else if ((extco = (hdmitx_rd_reg(HDMITX_DWC_FC_AVICONF2) >> 4) & 7) <= 7)
		color = colorimetry[extco + 3];
	pr_info(VID "Gamut\n\tColorimetry %s\n\tTransfer %s\n",
		color, eotf[(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB00) & 7)]);

	pr_info(VID "DV type %s\n", dv_type[hdmitx_get_cur_dv_st() & 3]);

	if (hdmitx_device.chip_type >= MESON_CPU_ID_SC2)
		pr_info(VID "PLL clock: 0x%08x, Vid clock div 0x%08x\n",
			hd_read_reg(P_ANACTRL_HDMIPLL_CTRL0),
			hd_read_reg(P_CLKCTRL_VID_PLL_CLK_DIV));
	else
		pr_info(VID "PLL clock: 0x%08x, Vid clock div 0x%08x\n",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	long ret_val;
	int force_hdr_new;
	if (kstrtol(buf, 10, &ret_val) >= 0)
		force_hdr_new = (int) ret_val;
	else
		force_hdr_new = 0;
	if (force_hdr_new != force_hdr) {
		force_hdr = force_hdr_new;
	...
	if (type == 1) {
		...
		for (i = 0; i < block_cnt; i++) {
			for (j = 0; j < 8; j++) {
				...
				line[16*3] = 0x0;
				pos = (i*8+j)*16*3;
				size += vfs_write(filp, line, 16*3, &pos);
			}
		}
	}
...
static int load_edid_data(unsigned int type, char *path)
{
	...
	spin_lock_irqsave(&hdmitx_device.edid_spinlock, flags);
	hdmitx_edid_clear(&hdmitx_device);
	hdmitx_edid_parse(&hdmitx_device);
	spin_unlock_irqrestore(&hdmitx_device.edid_spinlock, flags);
	if (hdmitx_device.edid_parsing) {
...
static void hdr_work_func(struct work_struct *work)
{
	...
	if (hdev->hdr_transfer_feature == T_BT709 &&
		...
		if (hdr_mute_frame) {
			...
			hdmitx_audio_mute_op(!hdev->hdmi_audio_off_flag);
		}
		hdmitx_sdr_hdr_uevent(hdev);
	}
}
...
static void hdmitx_set_drm_pkt(struct master_display_info_s *data)
{
	...
	unsigned char DRM_HB[3] = {0x87, 0x1, 26};
	static unsigned char last_eotf = 0;
	unsigned char DRM_DB[26] = {0x0};
	DRM_DB[0] = last_eotf;
	unsigned long flags = 0;
...
/* if using VSIF/DOVI, then only clear DV_VS10_SIG, else disable VSIF */
	...
	if ((hdev->hdr_transfer_feature == T_BT709 ||
		...
		if (hdev->hdr_transfer_feature == T_BT2020_10) {
			DRM_DB[0] = 0x01; /* HDR gamma */
			hdmitx_device.hwop.setpacket(HDMI_PACKET_DRM,
				DRM_DB, DRM_HB);
			hdmitx_device.hwop.cntlconfig(&hdmitx_device,
				CONF_AVI_BT2020, SET_AVI_BT2020);
		} else if (hdev->sdr_hdr_feature == 0) {
			hdmitx_device.hwop.setpacket(HDMI_PACKET_DRM,
				NULL, NULL);
			hdmitx_device.hwop.cntlconfig(&hdmitx_device,
				CONF_AVI_BT2020, SET_AVI_BT2020);
	...
	/* print the data but only if changed */
	if (data->features != hdr_features) {
		char* eotf[] = {"SDR", "HDRgamma", "HDR10", "HLG"};
		...
		if (((hdmitx_rd_reg(HDMITX_DWC_FC_AVICONF1) & 0xc0) >> 6) < 0x3)
			pr_info(VID "Colorimetry: %s\n",
			colour_str[(hdmitx_rd_reg(HDMITX_DWC_FC_AVICONF1) & 0xc0) >> 6]);
		else
			pr_info(VID "Colorimetry: %s\n",
			colour_str[((hdmitx_rd_reg(HDMITX_DWC_FC_AVICONF2) & 0x70) >> 4) + 3]);
		
		pr_info(VID "HDR data: EOTF: %s\n", eotf[(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB00) & 7)]);

		if (hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB00) > 0) {
			if (data->present_flag) {
				if (data->present_flag & 2)
					pr_info(VID "Using default master display data");
				else if (data->present_flag & 4)
					pr_info(VID "Using custom master display data");
				else
					pr_info(VID "Using master display data from frame header");
				pr_info(VID "Master display colours:\nPrimary one 0.%04d,0.%04d, two 0.%04d,0.%04d, three 0.%04d,0.%04d\nWhite 0.%04d,0.%04d, Luminance max/min: %d,0.%04d\n",
					...
					(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB18) | (hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB19) << 8)),
					(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB20) | (hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB21) << 8)));
				pr_info(VID "Max content luminance: %d, Max frame average luminance: %d\n",
					(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB22) | (hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB23) << 8)),
					(hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB24) | (hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB25) << 8)));
			} else pr_info(VID "Not using master display info\n");
		}
	}
	hdr_features = data->features;
	last_eotf = hdmitx_rd_reg(HDMITX_DWC_FC_DRM_PB00) & 0x3;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (hdev->para) {
		...
		pos += snprintf(buf + pos, PAGE_SIZE, "cur_video_param->colordepth: %s\n",
				conf);
		switch (hdev->para->cs) {
		...
		pos += snprintf(buf + pos, PAGE_SIZE, "para->colorspace: %s\n",
				conf);
	}
	char* pix_fmt[] = {"RGB","YUV422","YUV444","YUV420"};
	char* eotf[] = {"SDR","HDRgamma","PQ","HLG"};
	char* dv_type[] = {"Not DV", "DV-Std", "DV-LL"};
	char* range[] = {"default","limited","full"};
	...
	if (hdev->para){
		pos += snprintf(buf+pos, PAGE_SIZE, "VIC: %d %s\n",
				hdmitx_device.cur_VIC, hdev->para->name);
	}
...
Colour range: %s\nEOTF: %s\nYCC colour range: %s\nColorimetry: %s\n",
	...
	pos += snprintf(buf + pos, PAGE_SIZE, "DV type %s\n",
		dv_type[hdmitx_get_cur_dv_st() & 3]);
...
const char *disp_mode_t[] = {
	...
	"800x480p60hz",
	"800x600p60hz",
	"852x480p60hz", /* not working Apr 2023 */
	"854x480p60hz", /* not working Apr 2023 */
	"1024x600p60hz",
	"1024x768p60hz",
	"1152x864p75hz",
	"1280x600p60hz", /* not working Apr 2023 */
	"1280x768p60hz",
	"1280x800p60hz",
	"1280x960p60hz",
	"1280x1024p60hz",
	"1360x768p60hz",
	"1366x768p60hz", /* not working Apr 2023 */
	"1400x1050p60hz",
	"1440x900p60hz",
	"1440x2560p60hz",
	"1600x900p60hz",
	...
	"1680x1050p60hz",
	"1920x1200p60hz",
	"2048x1080p24hz", /* not working Apr 2023 */
	"2160x1200p90hz",
	"2400x1200p90hz", /* not working Apr 2023 */
	"2560x1080p60hz", /* not working Apr 2023 */
	"2560x1080p75hz", /* not working Apr 2023 */
	"2560x1440p60hz", /* not working Apr 2023 */
	"2560x1600p60hz",
	"3440x1440p60hz",
	"3840x1080p60hz",
	NULL
...
static int is_interlaced_fmt(char *mode)
{
	int i;
	static char const *interlaced_modes[] = {
		"1080i50hz",
		"1080i60hz",
		NULL
	};

	for (i = 0; interlaced_modes[i]; i++) {
		if (strstr(mode, interlaced_modes[i]))
			return 1;
	}
	return 0;
}

static int filter_sc2_fmt(char *mode)
{
        int i;
        static char const *problem_modes[] = {
		"1366x768p60hz", // Checked on LG and Panasonic 2017 which have some dumb EDIDs
		"2560x1080p75hz",
		"2400x1200p90hz",
		"2560x1080p60hz",
		"2560x1080p75hz",
        ...
        for (i = 0; problem_modes[i]; i++) {
                if (strstr(mode, problem_modes[i]))
                        return 1;
        }
        return 0;
}

/* print just the CTA modes */
static ssize_t show_cta_cap(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int i, pos = 0;
...
{
	int pos = 0;
	pos += show_cta_cap(dev, attr, buf);
	pos += show_vesa_cap(dev, attr, buf + pos);
	return pos;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; disp_mode_t[i]; i++) {
		/* 3D is not supported under 4k modes and we don't want to support interlaced either */
		if (strstr(disp_mode_t[i], "2160p") ||
			strstr(disp_mode_t[i], "smpte") ||
			is_interlaced_fmt(disp_mode_t[i]))
			continue;
		vic = hdmitx_edid_get_VIC(&hdmitx_device,
			disp_mode_t[i], 0);
		if (vic == HDMI_Unknown)
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < prxcap->AUD_count; i++) {
		...
		case CT_MAT:
		case CT_DST:
			pos += snprintf(buf + pos, PAGE_SIZE, "AFCDepValue 0x%x\n",
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (hdr_status_pos == 1) {
		...
		if (hdev->hdr_color_feature == C_BT2020) {
			if (hdev->hdr_transfer_feature == T_HLG)
				pos += snprintf(buf + pos, PAGE_SIZE,
				"HLG\n");
			else if (hdev->hdr_transfer_feature == T_BT2020_10)
				pos += snprintf(buf + pos, PAGE_SIZE,
				"BT2020SDR\n");
			return pos;
		}
	...
	struct device_attribute *attr, char *buf)
{
	   ...
	   if (hdmitx_device.speaker_layout >= 0){
			pos += snprintf(buf + pos, PAGE_SIZE,
				"speaker layout = %s\nhdmi_ch = %d\n", 
				speaker_layouts[hdmitx_device.speaker_layout], hdmitx_device.hdmi_ch);
	   }
	   return pos;
}
...
{
	int ret;
	ret = kstrtoint(buf, 0, &hdmitx_device.hdmi_ch);
	/* limit to CEA-861-D values */
	if (hdmitx_device.hdmi_ch > 0x13)
		hdmitx_device.hdmi_ch = 0;
	pr_info("sysfs set hdmi_ch to %d", hdmitx_device.hdmi_ch);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info(VID "Aspect ratio set to 0x%02x/0x%02x\n",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	strcat(hdmitx_device.fmt_attr, "now");
	set_disp_mode_auto();

	return count;
}

static DEVICE_ATTR(output_rgb, 0220, NULL, store_output_rgb);
static DEVICE_ATTR(force_hdr, 0664, show_force_hdr,store_force_hdr);
static DEVICE_ATTR(aspect, 0664, show_aspect, store_aspect);
...
static DEVICE_ATTR(cta_cap, 0444, show_cta_cap, NULL);
	...
	unsigned long cmd, void *para)
{
	...
	if (audio_param->sample_rate != n_rate) {
                pr_info(AUD "aout notify sample rate %d was %d\n",
                                map_fs[n_rate].rate,
	...
	if (audio_param->type != cmd) {
                 pr_info(AUD "aout notify format %s was %s\n",
                        (cmd == CT_DTS_HD_MA) ? \
	...
	if (audio_param->sample_size != n_size) {
		pr_info(AUD "aout notify sample size %d was %d\n", n_size, audio_param->sample_size);
	...
	if (audio_param->channel_num !=
        ...
        pr_info(AUD "hdmi_ch: %d speaker_layout: %d\n", hdmitx_device.hdmi_ch, hdmitx_device.speaker_layout);
        if (hdmitx_device.speaker_layout != hdmitx_device.hdmi_ch){
                if ((hdmitx_device.hdmi_ch == 0 && substream->runtime->channels == 2) || hdmitx_device.hdmi_ch > 0) {
                        hdmitx_device.speaker_layout = hdmitx_device.hdmi_ch;
                        hdmitx_device.audio_param_update_flag = 1;
                }
        }

       if (hdmitx_device.tx_aud_cfg == 2) { // never set to 2??
...
static void hdmitx_get_edid(struct hdmitx_dev *hdev)
{
	...
	mutex_lock(&getedid_mutex);
	if (load_edid_data(0, "/lib/firmware/edid") < 0) {
		...
		if (!check_dvi_hdmi_edid_valid(hdev->EDID_buf)) {
			/* Assume corrupt or all zeros. Either way does not matter */
			int tries;
			tries = 5;
			while (!check_dvi_hdmi_edid_valid(hdev->EDID_buf1) && tries > 0) {
				pr_info("Trying to read EDID %d tries left\n", tries--);
				hdev->hdmitx_event |= HDMI_TX_HPD_PLUGIN;
				msleep(100);
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(dev, &dev_attr_cta_cap);
...
static int amhdmitx_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev, &dev_attr_cta_cap);
