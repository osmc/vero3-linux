...
static int set_disp_mode_auto(void)
{
	...
	/* nothing */
        ...
        /* keep hdcp happy when transitioning */
        ...
        if (hdcp_ver)
                hdmitx_set_hdcp_mode(0);
                msleep(100);
                hdmitx_set_hdcp_mode(hdcp_ver);
...
unsigned char hdmitx_hdcp_support(struct hdmitx_dev *hdev)
{
	unsigned char hdcp_ver = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (hdmitx_device.lstore < 0x10) {
		...
		hdmitx_device.lstore = hdmitx_hdcp_support(&hdmitx_device);
	}
...
void hdmitx_set_hdcp_mode(int hdcp_mode)
{
       /* todo: change buf to int */
       pr_info(SYS "hdcp: set mode as %d\n", hdcp_mode);
       hdmitx_device.hwop.cntlddc(&hdmitx_device, DDC_HDCP_MUX_INIT, 1);
...
/* temporarily mark, no use SW I2C any more */
	...
	hdmitx_device.hwop.cntlddc(&hdmitx_device, DDC_PIN_MUX_OP, PIN_MUX);
       ...
       ret = kstrtoint(buf, 10, &hdcp_mode);
       if (ret < 0) {
               pr_info("invalid hdcp mode\n");
               return count;

	}

	hdmitx_set_hdcp_mode(hdcp_mode);
...
enum hdcp_ver_e hdmirx_hdcp_support(struct hdmitx_dev *hdev)
{
	uint32_t ver = 0U;
...
{
	int pos = 0;
	enum hdcp_ver_e ver = HDCPVER_NONE;
...
static int amhdmitx_get_dt_info(struct platform_device *pdev)
{
	...
	hdmitx_device.irq_vsync = platform_get_irq_byname(pdev, "hdmitx_vsync");
	if (hdmitx_device.irq_vsync == -ENXIO)
		pr_info("%s: ERROR: hdmitx vsync irq No not found\n",
		       __func__);
	pr_info(SYS "vsync irq = %d\n", hdmitx_device.irq_vsync);
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	edidinfo_attach_to_vinfo(&hdmitx_device);
	hdmitx_wrap_init(&hdmitx_device);
