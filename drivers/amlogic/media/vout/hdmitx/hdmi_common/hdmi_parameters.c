...
static struct hdmi_format_para fmt_para_vesa_2560x1080p75_64x27 = {
	.vic = HDMIV_2560x1080p75hz,
	.name = "2560x1080p75hz",
	.sname = "2560x1080p75hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
	.tmds_clk_div40 = 0,
...
/* the following modes for 1920x1200 are calculated for a Barco G50-W8 */
static struct hdmi_format_para fmt_para_vesa_1920x1200p50_8x5 = {
	.vic = HDMIV_1920x1200p50hz,
	.name = "1920x1200p50hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
static struct hdmi_format_para fmt_para_vesa_1920x1200p48_8x5 = {
	.vic = HDMIV_1920x1200p48hz,
	.name = "1920x1200p48hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
static struct hdmi_format_para fmt_para_vesa_1920x1200p30_8x5 = {
	.vic = HDMIV_1920x1200p30hz,
	.name = "1920x1200p30hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
static struct hdmi_format_para fmt_para_vesa_1920x1200p25_8x5 = {
	.vic = HDMIV_1920x1200p25hz,
	.name = "1920x1200p25hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
static struct hdmi_format_para fmt_para_vesa_1920x1200p24_8x5 = {
	.vic = HDMIV_1920x1200p24hz,
	.name = "1920x1200p24hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
/* 3D Frame Packing Modes */

static struct hdmi_format_para fmt_para_1920x1080fp24_16x9 = {
	.vic = HDMI_1920x1080p24_16x9,
	.name = "1920x1080fp24hz",
	.sname = "1080fp24hz",
	.pixel_repetition_factor = 0,
	.progress_mode = 1,
	.scrambler_en = 0,
...
static struct hdmi_format_para fmt_para_1280x720fp60_16x9 = {
	.vic = HDMI_1280x720p60_16x9,
	.name = "1280x720fp60hz",
	.sname = "720fp60hz",
	.pixel_repetition_factor = 0,
...
static struct hdmi_format_para fmt_para_1280x720fp50_16x9 = {
	.vic = HDMI_1280x720p50_16x9,
	.name = "1280x720fp50hz",
	.sname = "720fp50hz",
	.pixel_repetition_factor = 0,
...
static struct hdmi_format_para *all_fmt_paras[] = {
	...
	&fmt_para_vesa_1920x1200p50_8x5,
	&fmt_para_vesa_1920x1200p48_8x5,
	&fmt_para_vesa_1920x1200p30_8x5,
	&fmt_para_vesa_1920x1200p25_8x5,
	&fmt_para_vesa_1920x1200p24_8x5,
	...
	&fmt_para_vesa_2560x1080p75_64x27,
...
void reset_all_fmt_paras()
{
	int i;
	for (i = 0; all_fmt_paras[i]; i++) {
		if (all_fmt_paras[i]->cta_vesa_default != NULL) {
			struct hdmi_format_para *cta_vesa_def = all_fmt_paras[i]->cta_vesa_default;
			memcpy(all_fmt_paras[i], cta_vesa_def, sizeof(struct hdmi_format_para));
			all_fmt_paras[i]->cta_vesa_default = NULL;
			kfree(cta_vesa_def);
			pr_info("edid: reset VIC %d, %s to default timings\n", all_fmt_paras[i]->vic,
...
struct hdmi_format_para *hdmi_match_dtd_paras(struct dtd *t)
{
	...
	/* Not going to consider interlaced, stereo or modes other than Digital Separate */
	if (t->flags & 0xE0 || ! (t->flags & 0x18) || t->h_active == 0)
		return NULL;

	/* Reset framerate to an integer */
	framerate = (t->pixel_clock * 20000 / (t->h_active + t->h_blank) * 100 /
		(t->v_active + t->v_blank) + 1) / 2;
	/* assume the display also supports the integer rate */
	if (framerate % 100 != 0) {
		framerate += framerate / 200;
		framerate = framerate / 100;
		t->pixel_clock = (unsigned short) (framerate * (t->h_active + t->h_blank) * (t->v_active + t->v_blank) / 10000);
		framerate *= 100;
	}

	/* find the nearest match for that mode */
	for (i = 0; all_fmt_paras[i]; i++) {
		if ((t->h_active == all_fmt_paras[i]->timing.h_active) &&
		    (t->v_active == all_fmt_paras[i]->timing.v_active) &&
			(framerate / 100 == (int) (all_fmt_paras[i]->timing.v_freq + 500)/1000)) {
			/* assume CTA resolutions are supported with CTA timings */
			if (all_fmt_paras[i]->vic < HDMITX_VESA_OFFSET) {
				pr_info("Mode is CTA - ignoring alternative timing\n");
				return all_fmt_paras[i];
			}
			/* does it need modifying to match the DTD? */
			if ((all_fmt_paras[i]->timing.pixel_freq / 10 != t->pixel_clock) ||
				...
				/* if no match except width x height x framerate, update parameters from the EDID */
				pr_info("Can't find timing exact match for %d x %d x %d\n", t->h_active, t->v_active, framerate / 100);
				struct hdmi_format_para *cta_vesa_def = kmalloc(sizeof(struct hdmi_format_para), GFP_KERNEL);
				if (cta_vesa_def == NULL)
					return NULL;
				memcpy(cta_vesa_def, all_fmt_paras[i], sizeof(struct hdmi_format_para));
				all_fmt_paras[i]->cta_vesa_default = cta_vesa_def;
				all_fmt_paras[i]->timing.pixel_freq = t->pixel_clock * 10;
				all_fmt_paras[i]->tmds_clk = t->pixel_clock * 10;
				all_fmt_paras[i]->hdmitx_vinfo.video_clk = t->pixel_clock * 10000;
				all_fmt_paras[i]->timing.v_freq = framerate;
				all_fmt_paras[i]->timing.h_freq = (t->pixel_clock * 20000 / (t->h_active + t->h_blank)
					+ 1) / 2;
				all_fmt_paras[i]->timing.h_blank = t->h_blank;
				all_fmt_paras[i]->timing.h_total = t->h_active + t->h_blank;
				all_fmt_paras[i]->hdmitx_vinfo.htotal = t->h_active + t->h_blank;
				all_fmt_paras[i]->timing.h_front = t->h_sync_offset;
				all_fmt_paras[i]->timing.h_sync = t->h_sync;
				all_fmt_paras[i]->timing.h_back = t->h_blank - t->h_sync_offset - t->h_sync;
				all_fmt_paras[i]->timing.v_blank = t->v_blank;
				all_fmt_paras[i]->timing.v_total = t->v_active + t->v_blank;
				all_fmt_paras[i]->hdmitx_vinfo.vtotal = t->v_active + t->v_blank;
				all_fmt_paras[i]->timing.v_front = t->v_sync_offset;
				all_fmt_paras[i]->timing.v_sync = t->v_sync;
				all_fmt_paras[i]->timing.v_back = t->v_blank - t->v_sync_offset - t->v_sync;
				all_fmt_paras[i]->timing.hsync_polarity = !!(t->flags & 0x02);
				all_fmt_paras[i]->timing.vsync_polarity = !!(t->flags & 0x04);

				pr_info("DTD changed timings for VIC %d %dx%d@%dHz", all_fmt_paras[i]->vic, t->h_active,
					t->v_active, framerate / 100);

#define PR(a) pr_info("edid: %s: %d\n", #a, all_fmt_paras[i]->a)

				PR(vic);
				PR(timing.pixel_freq);
				PR(tmds_clk);
				PR(hdmitx_vinfo.video_clk);
				PR(timing.h_freq);
				PR(timing.v_freq);
				PR(timing.h_blank);
				PR(timing.h_front);
				...
				PR(timing.v_front);
				PR(timing.v_sync);
				PR(timing.hsync_polarity);
				PR(timing.vsync_polarity);
				pr_info("Backup tmds clock %d\n", ((struct hdmi_format_para*) (all_fmt_paras[i]->cta_vesa_default))->tmds_clk);
				
			} else
				pr_info("Found timings for VIC %d %dx%d@%dHz", all_fmt_paras[i]->vic, all_fmt_paras[i]->timing.h_active,
					all_fmt_paras[i]->timing.v_active, all_fmt_paras[i]->timing.v_freq / 1000);
			return all_fmt_paras[i];
		}
	}
	return NULL;
	...
	if (strlen(mode)) {
		...
		for (i = 0; all_fmt_paras[i]; i++) {
			...
			if (strncmp(all_fmt_paras[i]->hdmitx_vinfo.name, mode_,
				strlen(all_fmt_paras[i]->hdmitx_vinfo.name)) == 0) {
