...
/* these numbers from AML don't match CVT standard */
...
/* these parameters suitable for Dell UltraSharp 2407WFP */
...
struct hdmi_format_para *hdmi_match_dtd_paras(struct dtd *t)
{
	...
	if (t->flags & 0xE0 || ! (t->flags & 0x18) || t->h_active == 0)
		return NULL;
	int framerate = (t->pixel_clock * 20000 / (t->h_active + t->h_blank) /
	...
	for (i = 0; all_fmt_paras[i]; i++) {
		...
		if ((t->h_active == all_fmt_paras[i]->timing.h_active) &&
		    (t->v_active == all_fmt_paras[i]->timing.v_active) &&
			(framerate == all_fmt_paras[i]->timing.vsync)) {
...
struct vinfo_s *hdmi_get_valid_vinfo(char *mode)
{
	...
	if (strlen(mode)) {
		...
		if (strstr(mode_, "420") != NULL)
			memcpy(strstr(mode_,"420"), "\0", 1);

		if (strstr(mode_, " ") != NULL)
			memcpy(strstr(mode_," "), "\0", 1);
