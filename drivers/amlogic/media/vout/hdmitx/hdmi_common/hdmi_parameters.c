...
struct hdmi_format_para *hdmi_match_dtd_paras(struct dtd *t)
{
	...
	for (i = 0; all_fmt_paras[i]; i++) {
		if ((abs(all_fmt_paras[i]->timing.pixel_freq / 10
		    - t->pixel_clock) <= t->pixel_clock / 1000) &&
	...
	if (t->flags & 0xE0 || ! (t->flags & 0x18) || t->h_active == 0)
		return NULL;
	framerate = (t->pixel_clock * 20000 / (t->h_active + t->h_blank) /
		(t->v_active + t->v_blank) + 1) / 2;
	pr_info("Can't find timing exact match for %d x %d x %d\n", t->h_active, t->v_active, framerate);
	for (i = 0; all_fmt_paras[i]; i++) {
		if ((t->h_active == all_fmt_paras[i]->timing.h_active) &&
		    (t->v_active == all_fmt_paras[i]->timing.v_active) &&
			(framerate == (int) (all_fmt_paras[i]->timing.v_freq + 500)/1000)) {
			...
			/* Dell 16:10 monitors RB mode not supported */
			if (all_fmt_paras[i]->vic == HDMIV_1920x1200p60hz) {
				pr_info("Using non-RB timings for 1920x1200");
				return all_fmt_paras[i];
			}
			...
			all_fmt_paras[i]->timing.v_freq = (t->pixel_clock * 20000 / (t->h_active + t->h_blank) *
				1000 / (t->v_active + t->v_blank) + 1) / 2;
			...
			pr_info("DTD changed timings for VIC %d", all_fmt_paras[i]->vic);
