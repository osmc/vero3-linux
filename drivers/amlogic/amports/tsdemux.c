			      ...
			      int isphybuf)
{
	...
	if (r > 0) {
		if (isphybuf)
			...
			dma_addr =
			    dma_map_single(amports_get_dma_device(), fetchbuf,
					   FETCHBUF_SIZE, DMA_TO_DEVICE);
			if (dma_mapping_error
			    (amports_get_dma_device(), dma_addr))
				return -EFAULT;

		}

		fetch_done = 0;
		/* Ensure fetchbuf  contents visible */
		wmb();
		 ...
		 struct vdec_s *vdec)
{
	...
	/* set PID filter */
	pr_info("tsdemux video_pid = 0x%x, audio_pid = 0x%x,", vid, aid);
	pr_info("sub_pid = 0x%x, pcrid = 0x%x\n", sid, pcrid);

	if (!enable_demux_driver()) {
		WRITE_DEMUX_REG(FM_WR_DATA,
				(((vid <
				   0x1fff) ? (vid & 0x1fff) | (VIDEO_PACKET <<
							       13) : 0xffff) <<
				 16) | ((aid <
					 0x1fff) ? (aid & 0x1fff) |
					(AUDIO_PACKET << 13) : 0xffff));
		WRITE_DEMUX_REG(FM_WR_ADDR, 0x8000);
		while (READ_DEMUX_REG(FM_WR_ADDR) & 0x8000)
			;
		WRITE_DEMUX_REG(FM_WR_DATA, (((sid < 0x1fff) ? (sid & 0x1fff)
					      | (SUB_PACKET << 13)
					      : 0xffff) << 16) | 0xffff);
		...
		/* enable TS demux */
		WRITE_DEMUX_REG(DEMUX_CONTROL, (1 << STB_DEMUX_ENABLE)
	...
	if (has_hevc_vdec() && is_hevc) {
		...
		if (vdec_single(vdec)) {
			...
			SET_VREG_MASK(HEVC_STREAM_CONTROL, (1 << 3) | (0 << 4));
	...
	WRITE_PARSER_REG(PARSER_CONFIG, (10 << PS_CFG_PFIFO_EMPTY_CNT_BIT)
	...
	r = vdec_request_irq(PARSER_IRQ, parser_isr, "tsdemux-fetch",
	...
	if (!enable_demux_driver()) {
		...
		r = vdec_request_irq(DEMUX_IRQ, tsdemux_isr, "tsdemux-irq",
				     (void *)tsdemux_irq_id);

		WRITE_DEMUX_REG(STB_INT_MASK,
				(1 << SUB_PES_READY) | (1 << NEW_PDTS_READY) |
...
static int limited_delay_check(struct file *file, struct stream_buf_s *vbuf,
			       ...
			       const char __user *buf, size_t count)
{
	...
	if (vbuf->max_buffer_delay_ms > 0 && abuf->max_buffer_delay_ms > 0
		...
		int vdelay = calculation_stream_delayed_ms(PTS_TYPE_VIDEO,
							   NULL, NULL);
		int adelay = calculation_stream_delayed_ms(PTS_TYPE_AUDIO,
							   NULL, NULL);
...
ssize_t drm_tswrite(struct file *file, struct stream_buf_s *vbuf,
		    struct stream_buf_s *abuf, const char __user *buf,
		    size_t count)
{
	s32 r;
	u32 realcount = count;
...
ssize_t tsdemux_write(struct file *file, struct stream_buf_s *vbuf,
		      struct stream_buf_s *abuf, const char __user *buf,
		      size_t count)
{
	s32 r;
