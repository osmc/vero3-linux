	...
	const struct ad7793_platform_data *pdata)
{
	if ((pdata->current_source_direction == AD7793_IEXEC1_IEXEC2_IOUT1 ||
		pdata->current_source_direction ==
	...
	unsigned int vref_mv)
{
	...
	ret = spi_write(st->sd.spi,
		(u8 *)&reset, sizeof(reset)/sizeof(reset[0]));
	if (ret < 0)
		goto out;
	...
	if (id != st->chip_info->id) {
		dev_err(&st->sd.spi->dev,
			"device ID query failed,id=0x%x\n", id);
		goto out;
	}
	...
	if ((st->chip_info->flags & AD7793_FLAG_HAS_GAIN))
		st->conf |= AD7793_CONF_GAIN(2); /*gain set*/
	...
	dev_info(&st->sd.spi->dev, "setup success\n");
	return 0;
out:
...
static ssize_t ad7793_show_status(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_STAT, 1, &status);
	len = sprintf(buf, "0x%x\n", status);
	return len;
}

static ssize_t ad7793_show_mode(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_MODE, 2, &mode);
	len = sprintf(buf, "0x%x\n", mode);
	return len;
}

static ssize_t ad7793_show_config(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_CONF, 2, &config);
	len = sprintf(buf, "0x%x\n", config);
	return len;
}

static ssize_t ad7793_store_config(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	ret = kstrtoint(buf, 0, &config);
	if (ret)
		return -EINVAL;
	st->conf = config;
	ad_sd_write_reg(&st->sd, AD7793_REG_CONF, 2, st->conf);
	return len;
}

static ssize_t ad7793_show_io(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_IO, 1, &io);
	len = sprintf(buf, "0x%x\n", io);
	return len;
}

static ssize_t ad7793_store_io(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ret = kstrtoint(buf, 0, &io);
	if (ret)
		return -EINVAL;
	ad_sd_write_reg(&st->sd, AD7793_REG_IO, 1, io);
	return len;
}

static ssize_t ad7793_show_offset(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_OFFSET, 2, &data);
	len = sprintf(buf, "0x%x\n", data);
	return len;
}

static ssize_t ad7793_store_offset(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	if (ret)
		return -EINVAL;
	ad_sd_write_reg(&st->sd, AD7793_REG_OFFSET, 2, data);
	return len;
}

static ssize_t ad7793_show_full_scale(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	ad_sd_read_reg(&st->sd, AD7793_REG_FULLSALE, 3, &data);
	len = sprintf(buf, "0x%x\n", data);
	return len;
}

static ssize_t ad7793_store_full_scale(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7793_state *st = iio_priv(indio_dev);
	...
	if (ret)
		return -EINVAL;
	ad_sd_write_reg(&st->sd, AD7793_REG_FULLSALE, 3, data);
	return len;
}

/* Code = (2(24) x AIN x GAIN)/Vref
 *  I = 0.21mA Vref = 126mV Gain = 4
 * AIN = (Code*126)/(16777216*4)
*/
static unsigned int temperature_code[] = {
	10307922,
	10747485,
	11184811,
	11621018,
...
static ssize_t ad7793_show_temperature(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	int len = 0;
	...
	if (ret < 0)
		return -EINVAL;
	size = ARRAY_SIZE(temperature_code);
	for (i = 0; i < size; i++) {
		if (data < temperature_code[i])
			break;
	}
	if (i == 0)
		temp = -20;
	else {
		temp = temperature[i-1]*10 +
			((data - temperature_code[i-1]) * 100/
			(temperature_code[i]-temperature_code[i-1]));
	}
	len = sprintf(buf, "%d\n", temp);
	return len;
}
...
static IIO_DEVICE_ATTR_NAMED(status,
		status, 0444,
		ad7793_show_status, NULL, 0);

static IIO_DEVICE_ATTR_NAMED(mode,
		mode, 0444,
		ad7793_show_mode, NULL, 0);

static IIO_DEVICE_ATTR_NAMED(config,
		config, 0666,
		ad7793_show_config, ad7793_store_config, 0);

static IIO_DEVICE_ATTR_NAMED(io,
		io, 0666,
		ad7793_show_io, ad7793_store_io, 0);

static IIO_DEVICE_ATTR_NAMED(offset,
		offset, 0666,
		ad7793_show_offset, ad7793_store_offset, 0);

static IIO_DEVICE_ATTR_NAMED(full_scale,
		full_scale, 0666,
		ad7793_show_full_scale, ad7793_store_full_scale, 0);

static IIO_DEVICE_ATTR_NAMED(temperature,
		temperature, 0444,
		ad7793_show_temperature, NULL, 0);
...
static struct attribute *ad7793_attributes[] = {
	...
	&iio_dev_attr_io.dev_attr.attr,
	&iio_dev_attr_offset.dev_attr.attr,
	&iio_dev_attr_full_scale.dev_attr.attr,
...
static struct ad7793_platform_data ad7793_default_pdata = {
	...
	.exitation_current = AD7793_IX_210uA,
	.current_source_direction = AD7793_IEXEC1_IOUT1_IEXEC2_IOUT2
};

static int ad7793_parse_dts(struct spi_device *spi)
{
	...
	if (!node) {
		dev_err(&spi->dev, "Only DT devices are supported\n");
		return -EINVAL;
	}
	gpio = of_get_named_gpio_flags(node, "ready-gpios", 0, NULL);
	if (gpio_is_valid(gpio)) {
		rc = gpio_request_one(gpio, GPIOF_DIR_IN, "ad7793");
		if (rc) {
			dev_err(&spi->dev,
				"gpio_request_one ready-gpios err\n");
			return -EINVAL;
		}
		gpio_direction_input(gpio);
		gpio_set_pullup(gpio, 1);
		dev_info(&spi->dev, "config detect miso gpio\n");
	}
	of_property_read_u32(node, "gpioirq", &gpioirq);
	spi->irq = irq_of_parse_and_map(node, 0);
	irqflag = AML_GPIO_IRQ(gpioirq, FILTER_NUM7, GPIO_IRQ_FALLING);
	gpio_for_irq(gpio, irqflag);
	return 0;
}
...
static int ad7793_probe(struct spi_device *spi)
{
	...
	dev_info(&spi->dev, "ad7793_probe......\n");
	pdata = &ad7793_default_pdata;
	ret = ad7793_parse_dts(spi);
	if (ret)
		return ret;
...
#ifdef CONFIG_OF
static const struct of_device_id ad7793_of_match[] = {
...
static struct spi_driver ad7793_driver = {
	.driver = {
		.name	= "ad7793",
		.of_match_table = ad7793_of_match,
		.owner	= THIS_MODULE,
	},
	.probe		= ad7793_probe,
