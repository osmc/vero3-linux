...
#define SEI_AltTransferCharacteristics 147
...
#define SEI_PREF_TRANSFER_MASK		  0x00000010
		...
		if (p+payload_size <= sei_buf+size) {
			...
			case SEI_ContentLightLevel:
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
					...
					hevc->content_light_level[1]);
				break;
			case SEI_AltTransferCharacteristics: {
				char preferred_transfer_characteristics = *p;
				u32 data;
				hevc->sei_present_flag |= SEI_PREF_TRANSFER_MASK;
				data = hevc->video_signal_type;
				data = data & 0xFFFF00FF;
				data = data | (preferred_transfer_characteristics << 8);
				if ((video_signal_type & 0xff00) >> 8 != preferred_transfer_characteristics)
					pr_info("hevc: preferred transfer characteristics set to 0x%02x\n",
						preferred_transfer_characteristics);
				video_signal_type = data;
				break;
			}
			default:
				break;
...
*/
	...
	if (hevc->video_signal_type & VIDEO_SIGNAL_TYPE_AVAILABLE_MASK) {
		...
		if (hevc->sei_present_flag & SEI_PREF_TRANSFER_MASK) {
			u32 data;
			data = vf->signal_type;
			data = data & 0xFFFF00FF;
			data = data | (video_signal_type & 0xFF00);
			vf->signal_type = data;
		} else
			video_signal_type = hevc->video_signal_type;
	}
...
static int vh265_event_cb(int type, void *data, void *op_arg)
{
	...
	if (type & VFRAME_EVENT_RECEIVER_RESET) {
		...
		if (index != 0xff
			...
			hevc_print(hevc, H265_DEBUG_DV,
			"query dv_enhance_exist for (pic 0x%p, vf 0x%p, poc %d index %d) flag => %d, %d aux sizd 0x%x\n",
			hevc->m_PIC[index],
			req->vf,
			hevc->m_PIC[index]->POC, index,
			req->dv_enhance_exist, hevc->m_PIC[index]->dv_enhance_exist, req->aux_size);
#else
...
static int __init amvdec_h265_driver_init_module(void)
{
	...
	if (is_meson_gxl_cpu()) {
		pr_debug("amvdec_h265: set mmu_enable_force for this device\n");
		mmu_enable_force = 1;
	}
