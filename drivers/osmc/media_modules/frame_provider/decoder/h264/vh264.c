...
static uint error_recovery_mode = 2;
...
static int get_max_dpb_size(int level_idc,
		int max_reference_frame_num, int picWidthInMbs, int frameHeightInMbs)
{
	int maxDpbFrames = 0;
	int maxDpbMbs = 0;
	...
	/* formulas according to constraints in Rec. ITU-T H.264 (04/2013), Section A.3.2 */
	maxDpbFrames = min(maxDpbMbs / (picWidthInMbs * frameHeightInMbs), DECODE_BUFFER_NUM_MAX);

	if (max_reference_frame_num > maxDpbFrames) {
		// just in case ... (shouldn't happen, see Rec. ITU-T H.264 (04/2013), Section G.7.4.2.1.1)
		maxDpbFrames = min(max_reference_frame_num, DECODE_BUFFER_NUM_MAX);
	}
 ...
 */
	...
	max_dpb_size = get_max_dpb_size(level_idc, max_reference_size, mb_width, mb_height);
	if (max_dpb_size > 15
