...
{
	return ((index * 2 + 1) << 16) | ((index * 2 + 1) << 8) | (index * 2);
}
 ...
 */
		...
		if ((reg & INTERLACE_FLAG) && ! force_prog) {	/* interlace */
...
MODULE_PARM_DESC(force_prog, "\ntreat frame as progressive");
module_param(force_prog, int, 0664);
