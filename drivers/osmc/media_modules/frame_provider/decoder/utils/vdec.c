...
s32 vdec_init(struct vdec_s *vdec, int is_4k)
{
	...
	pr_info("vdec_init, dev_name:%s, vdec_type=%s  id = %d port type %d\n",
		dev_name, vdec_type_str(vdec), vdec->id, vdec->port->type);
	...
	/* todo */
	if (!vdec_dual(vdec)) {
		p->use_vfm_path = is_support_no_parser() ? !is_mult_inc(p->port->type) : vdec_stream_based(vdec);
	}
	...
	if (p->use_vfm_path) {
		...
		if (strncmp("disable", vfm_path, strlen("disable"))) {
			...
			snprintf(vdec->vfm_map_id, VDEC_MAP_NAME_SIZE,
				"vdec-map-%d", vdec->id);
		} else if (p->frame_base_video_path == FRAME_BASE_PATH_AMVIDEO) {
			if (vdec_secure(vdec)) {
				snprintf(vdec->vfm_map_chain, VDEC_MAP_NAME_SIZE, "%s %s", vdec->vf_provider_name, "amlvideo amvideo");
			} else {
				snprintf(vdec->vfm_map_chain, VDEC_MAP_NAME_SIZE, "%s %s", vdec->vf_provider_name, "ppmgr deinterlace amlvideo amvideo");
			}
			snprintf(vdec->vfm_map_id, VDEC_MAP_NAME_SIZE, "vdec-map-%d", vdec->id);
		} else if (p->frame_base_video_path ==
...
unsigned long vdec_ready_to_run(struct vdec_s *vdec, unsigned long mask)
{
	...
	if (vdec_core_with_input(mask)) {
		...
		if (input && input_stream_based(input) && !input->eos) {
			...
			if (!vdec_check_rec_num_enough(vdec)) {
				vdec->need_more_data |= VDEC_NEED_MORE_DATA;
#ifdef VDEC_DEBUG_SUPPORT
				inc_profi_count(mask, vdec->input_underrun_count);
				if (step_mode & 0x200) {
