...
void rtw_os_recv_indicate_pkt(_adapter *padapter, _pkt *pkt, union recv_frame *rframe)
{
	...
	if (pkt) {
		...
		if (!skb_queue_empty(&precvpriv->rx_napi_skb_queue)
			...
			/*
			 * This runs in rtw_recv_thread (process context, see
			 * CONFIG_RECV_THREAD_MODE). napi_recv() here delivers
			 * inline via netif_receive_skb() from a kernel thread,
			 * which is the wrong context and was livelocking the
			 * NET_RX softirq on one CPU (the silent WiFi freeze).
			 * Flush the residual queue through netif_rx() instead:
			 * context-safe from any caller, and the backlog has a
			 * budget so it can't spin. The NAPI poll path
			 * (rtw_recv_napi_poll) is left untouched.
			 */
			_pkt *qskb;

			while ((qskb = skb_dequeue(&precvpriv->rx_napi_skb_queue)))
				rtw_netif_rx(padapter->pnetdev, qskb);
		}
#endif
