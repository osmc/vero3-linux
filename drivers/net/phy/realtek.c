...
static int rtl8211f_config_init(struct phy_device *phydev)
{
	...
	pr_info("NAZARKO: Realtek set TX delay");
	phy_write(phydev, RTL8211F_PAGE_SELECT, 0xd08);
	reg = phy_read(phydev, 0x11);
	reg |= 0x100;
	phy_write(phydev, 0x11, reg);
...
{
	/* We already have our PHY set up as we want with 2ns skew for TX
	*  Allowing this to run will cause unpredictable behaviour */
	return 0;
