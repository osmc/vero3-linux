...
static void rtl8211f_set_rxtx_delay(struct phy_device *phydev)
{
	int has_rx_delay = 0;
	int has_tx_delay = 0;
	u16 reg;

	pr_info("%s: setting phy-mode to %s\n", phydev->drv->name, phy_modes(phydev->interface));

	if (phydev->interface == PHY_INTERFACE_MODE_RGMII) {
		has_rx_delay = external_rx_delay;
		has_tx_delay = external_tx_delay;
	} else {
		if (phydev->interface == PHY_INTERFACE_MODE_RGMII_TXID || phydev->interface == PHY_INTERFACE_MODE_RGMII_ID) {
			/* enable TX-delay for rgmii-id and rgmii-txid, otherwise disable it */
			has_tx_delay = 1;
		}
		if (phydev->interface == PHY_INTERFACE_MODE_RGMII_RXID || phydev->interface == PHY_INTERFACE_MODE_RGMII_ID) {
			/* enable RX-delay for rgmii-id and rgmii-rxid, otherwise disable it */
			has_rx_delay = 1;
		}
	}

	pr_debug("tx_delay: %d, rx_delay: %d\n", has_tx_delay, has_rx_delay);

	phy_write(phydev, RTL8211F_PAGE_SELECT, 0xd08);

	/* set/reset 2 ns TX delay */
...
static int rtl8211f_config_init(struct phy_device *phydev)
{
	...
	rtl8211f_set_rxtx_delay(phydev);
	...
	if (is_meson_sc2_cpu() && (mac_addr[0] != 0x94 || mac_addr[1] != 0xCC || mac_addr[2] != 0x04 || mac_addr[4] != 0x60)) {
		pr_err("WARNING: This is not an officially supported device. Proceed at your own risk\n");
	}
#endif
