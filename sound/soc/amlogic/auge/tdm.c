                                     ...
                                     struct snd_ctl_elem_value *ucontrol)
{
	...
	struct aml_chmap *am_map = info->private_data;
    int res = 0, channel;

    if (mutex_lock_interruptible(&am_map->chmap_lock))
        return -EINTR;
...
/*    if (runtime->channels != 8)
    ...
    {
        ucontrol->value.integer.value[7 - channel] = channel_allocations[am_map->chmap_layout].speakers[channel];
    }

unlock:
    mutex_unlock(&am_map->chmap_lock);
    return res;
}
                                     ...
                                     struct snd_ctl_elem_value *ucontrol)
{
    ...
    am_map->chmap_layout = matched_layout;
			       ...
			       struct snd_soc_dai *cpu_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		enum aud_codec_types aud_codec_type;

		aud_codec_type = spdif_get_codec();
		if (aud_codec_type == AUD_CODEC_TYPE_STEREO_PCM &&
			runtime->channels > 2)
			aud_codec_type = AUD_CODEC_TYPE_MULTI_LPCM;
		...
		if (p_tdm->i2s2hdmitx) {
			...
			if (aud_codec_type == AUD_CODEC_TYPE_MULTI_LPCM)
			aout_notifier_call_chain(AOUT_EVENT_IEC_60958_PCM,
						 substream);
			else
				pr_warn("%s(), i2s2hdmi with wrong fmt,codec_type:%d\n",
					__func__, aud_codec_type);
		}
		...
		if (!aml_dai_tdm_chmap_kctrl_get(substream))
		{
			...
			am_map = info->private_data;
			if (am_map == NULL) {
				am_map = (struct aml_chmap*)kzalloc(sizeof(struct aml_chmap), GFP_KERNEL);
				info->private_data = am_map;
			}

			mutex_init(&am_map->chmap_lock);
		}
...
#define AML_DAI_TDM_RATES		(SNDRV_PCM_RATE_32000_192000)
