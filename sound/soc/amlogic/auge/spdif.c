...
{
	spdif_priv[0]->codec_type = IEC958_mode_codec;
...
static int aml_spdif_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		p_spdif->codec_type = IEC958_mode_codec;
		pr_info("%s opening spdif id %d IEC codec type %d\n",
			__func__, p_spdif->id, (int)p_spdif->codec_type);
	} else {
