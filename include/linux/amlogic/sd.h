...
#define MESSAGE_BUF_SIZE			512
	...
	u32 error_flag;
	/*
	 * OSMC: cumulative controller error / recovery counters, surfaced via
	 * the "emmc_errors" sysfs node.  These catch transient CRC/timeout
	 * events that the retry machinery recovers from silently and that
	 * therefore never reach the block layer (so never show up in dmesg).
	 * err_fatal counts commands that exhausted every retry.  Only updated
	 * outside tuning (host->is_tunning == 0), so tuning-window scan errors
	 * are not counted.
	 */
	u32 err_rsp_crc;
	u32 err_dat_crc;
