...
struct hdmitx_inf_param {
	int vic;
	char color_attr[16]; /* hdmi CS/CD string */
	unsigned int frac_rate_policy; /* 0.1% shift, 1080p60/59.94hz */
	unsigned int hdcp_mode; /* hdcp mode, 0/1/2 */
	unsigned int hdr_mode; /* hdr mode */
};
...
struct hdmitx_dev {
	...
	unsigned int sspll;
	/* configure for I2S: channel count and channel mask */
...
extern unsigned int hdmitx_rd_check_reg(unsigned int addr,
	...
	unsigned int mask);
extern unsigned char hdmitx_hdcp_support(struct hdmitx_dev *hdev);
extern void hdmitx_set_hdcp_mode(int hdcp_mode);
